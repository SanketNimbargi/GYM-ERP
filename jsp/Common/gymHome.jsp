<%@taglib uri="/struts-tags" prefix="s"%>
 <!-- <META http-equiv="REFRESH" content="5"> -->
<!-- Developed by: Shailesh Halor
	 Date: 03/09/2014
	 Purpose: Personal Trainer Registration
-->
<!-- 	<div class="notification_header" id="1" onclick="showNotification(this);"><span class="required"><s:property value="session.pendingFeesList.size()"/></span> Pending Fees</div>
				<div class="notification_header" id="11" onclick="showNotification(this);"><span class="required"><s:property value="session.cancelPendingFeesList.size()"/></span> Cancel Mem Pending</div>
				<div class="notification_header" id="2" onclick="showNotification(this);"><span class="required"><s:property value="session.reNewList.size()"/></span> Re-Join</div>
				<div class="notification_header" id="3" onclick="showNotification(this);"><span class="required"><s:property value="session.birthDayList.size()"/></span> BirthDay Today</div>
				<div class="notification_header" id="4" onclick="showNotification(this);"><span class="required"><s:property value="session.anniversaryDayList.size()"/></span> Anniversary Today</div>
				<div class="notification_header" id="5" onclick="showNotification(this);"><span class="required"><s:property value="session.attendanceList.size()"/></span> Attendance</div>
				<div class="notification_header" id="6" onclick="showNotification(this);"><span class="required"><s:property value="session.appointmentList.size()"/></span> Today Appointment</div>
				<div class="notification_header" id="7" onclick="showNotification(this);"><span class="required"><s:property value="session.enquirytList.size()"/></span> Enquiry</div>
				<div class="notification_header" id="8" onclick="showNotification(this);"><span class="required"><s:property value="session.inactiveMemberList.size()"/></span> Inactive Members</div>
				<div class="notification_header" id="9" onclick="showNotification(this);"><span class="required"><s:property value="session.activeMemberList.size()"/></span> Active Members</div>
			
				<div class="notification_header" id="10" onclick="showNotification(this);">Daily Attendance</div> -->
<!-- Java Script -->

<head>
<link rel="stylesheet" href="/css/jquery-ui.css">
<script src="/js/jquery-1.10.2.js"></script>
<script src="/js/jquery-ui.js"></script>
<!-- This Java Script should be used for TTS. Text To Speech-->
<%-- <script type="text/javascript" src="<%=request.getContextPath()%>/js/tts/speakClient.js"></script> --%>

<style>
body {
	font-family: "Trebuchet MS", "Helvetica", "Arial",  "Verdana", "sans-serif";
	font-size: 62.5%;
}
.modal {
  display: none; /* Hidden by default */
  position: fixed; /* Stay in place */
  z-index: 9; /* Sit on top */
  /* padding-top: 100px; /* Location of the box */ 
  left: 60%;
  top: 60%;
  width: auto; /* Full width */
  height: auto; /* Full height */
  overflow: auto; /* Enable scroll if needed */
  background-color: rgb(0,0,0); /* Fallback color */
   background-color: rgba(0,0,0,0.4); /* Black w/ opacity */
}

/* Modal Content */
.modal-content {
  background-image: linear-gradient(to right bottom, #cceeff , #bfbfbf,#cceeff, #bfbfbf, #bfbfbf,#cceeff , #bfbfbf,#cceeff, #bfbfbf, #bfbfbf);
  margin: auto;
  font-weight: bold;
  padding: 20px;
  border: 1px solid #888;
  width: 85%;
}

/* The Close Button */
.close {
  color: red;
  float: right;
  font-size: 28px;
  font-weight: bold;
}

.close:hover,
.close:focus {
  color: #000;
  text-decoration: none;
  cursor: pointer;
}
.input-border{
    
    border: 1px solid grey ; 
    border-radius : 3px;
    width:50%;
    height: 25px;

}

.form{
    
    text-align: center;
    width:400px;
    color:black;
    font-family: Arial, Helvetica, sans-serif;
    line-height: 35px;
    background-image: linear-gradient(to right bottom, #050c37, #003f72, #0076a4, #00b0c7, #12ebd8);
}

</style>
<script>
$(function() {
$( "#dialog" ).dialog();
});
</script>
</head>
<body onclick="closeModel()">
 <script type="text/javascript">
 <%-- //speak('hello world!'); //TTS Method --%>
 
 var bindCallbackMethods=function(){
	 $(document).bind("ajaxSend", function(){
		 $("#mainContainer").attr("onclick","return false;");
			$("#wait").css("display", "block");
		}).bind("ajaxComplete", function(){
			$("#wait").css("display", "none");$("#mainContainer").prop("onclick",null);
		}); 
 };
 
 var unbindCallbackMethods=function(){
	 $(document).unbind('ajaxComplete').unbind('ajaxSend');  //for chrome
 };
 bindCallbackMethods();
  	
  	/* $(document).bind("ajaxSend", function(){
	 $("#mainContainer").attr("onclick","return false;");
		$("#wait").css("display", "block");
	}).bind("ajaxComplete", function(){
		$("#wait").css("display", "none");$("#mainContainer").prop("onclick",null);
	}); */

/*  $(document).ajaxStart(function()   { 	
	 $("#mainContainer").attr("onclick","return false;");
	$("#wait").css("display", "block");
	//$("#mainContainer").attr("onclick","return false;");
	
	}); */ 
//$(document).ajaxComplete(function(){ $("#wait").css("display", "none");  });
 </script>
<script type="text/javascript">

var today = new Date();

var dd = today.getDate();
var mm = today.getMonth()+1; //January is 0!

var yyyy = today.getFullYear();
if(dd<10){
		 dd='0'+dd
} 
if(mm<10){
		 mm='0'+mm
} 
today = yyyy+"-"+mm+"-"+dd;
var oneday=1000*60*60*24;
var left=false;
var notificationStatus="hide";
	$("#document").ready(function(){		
		
		$.getJSON('sendMessageToMemberSupposedToGiveFees',{status:'yes'}).done(function(data){	
			console.log(data);
		});
		
		setNotificationLists(notificationStatus); 
		$("#toggle").click(function() {			  
			notificationStatus=(notificationStatus=="hide")?"show":"hide"; 
			setNotificationLists(notificationStatus); 
		});
		<%--//speak('Hello World! I am very happy. Thank you.',{ pitch: 100,wordgap:15 }); //TTS Method
		//speak('hello Tinaa!', { pitch: 100,wordgap:15 }); //TTS Method --%>
	
     		var setStatusColor=function(status){
     			if(status.substring(0,6)=="online")
     				$("#essl1").css("color","rgb(0, 160, 0)");
     			else
     				$("#essl1").css("color","rgb(255, 0, 0)");
     		};
     		 /*  $.getJSON('fetchAllNotificationList',{})			//Json To Fetch Data
  			.done(function(data)
  			{
  			}); */
  			$("#membersNameAutocomplete").keyup(function(){			
  				autoComplete("membersNameAutocomplete","displayMemberId",onMemberIdChange);
  			});
  			

  			
  			
  			
  			$.ajax({url:'fetchAllNotificationList',		//Json To Fetch Data
	    		success:function(data)
			{						
	    			var notificationLists=$("#notificationLists");
					notificationLists.empty();
					var timeAttendence='<%=application.getAttribute("timeAttendence")%>';	
					if(timeAttendence=="true"){
						var status='<%=application.getAttribute("esslDBStatus")%>';						
						$('<div class="notification_header" id="13" onclick="showNotification(this);">Essl status :<span class="required" id="essl1">'+status+'</span> </div>').appendTo(notificationLists);
						setStatusColor(status);
					}	
					$('<div class="notification_header" id="13"><a href="searchMemberDetailsByValues"> Search Member</a></div>').appendTo(notificationLists);
					$('<div class="notification_header" id="1" onclick="showNotification(this);"><span class="required" id="s1">'+data.pendingFeesList.length+'</span> Pending Fees</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="32" onclick="showNotification(this);"><span class="required" id="s32">'+data.pendingFeesRegularFollowUpList.length+'</span> Pending Follow Up</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="11" onclick="showNotification(this);"><span class="required" id="s11">'+data.cancelPendingFeesList.length+'</span> Pending Fees By <br/>Cancel Mem </div>').appendTo(notificationLists);
					$('<div class="notification_header" id="2" onclick="showNotification(this);"><span class="required" id="s2">'+data.reNewList.length+'</span> By Package EndDate</div>').appendTo(notificationLists); /* Re-Join */
					$('<div class="notification_header" id="3" onclick="showNotification(this);"><span class="required" id="s3">'+data.birthDayList.length+'</span> Today\'s BirthDay </div>').appendTo(notificationLists);
					$('<div class="notification_header" id="4" onclick="showNotification(this);"><span class="required" id="s4">'+data.anniversaryDayList.length+'</span> Today\'s Anniversary </div>').appendTo(notificationLists);
					$('<div class="notification_header" id="5" onclick="showNotification(this);"><span class="required" id="s5">'+data.attendanceList.length+'</span> Absent Mem List</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="6" onclick="showNotification(this);"><span class="required" id="s6">'+data.appointmentList.length+'</span> Today\'s Appointment</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="31" onclick="showNotification(this);"><span class="required" id="s31">'+data.regularFollowUpList.length+'</span> Regular Follow Up</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="7" onclick="showNotification(this);"><span class="required" id="s7">'+data.enquirytList.length+'</span> Enquiry Follow Up</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="8" onclick="showNotification(this);"><span class="required" id="s8">'+data.inactiveMemberList.length+'</span> Inactive Members</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="9" onclick="showNotification(this);"><span class="required" id="s9">'+data.activeMemberList.length+'</span> Active Members</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="20" onclick="showNotification(this);"><span class="required" id="s20">'+data.cancelMemberList.length+'</span> Canceled Members</div>').appendTo(notificationLists);

					$('<div class="notification_header" id="30" onclick="showNotification(this);"><span class="required" id="s30">'+data.enquiredMemberList.length+'</span> Enquired Members</div>').appendTo(notificationLists);
					//$('<div class="notification_header" id="31" onclick="showNotification(this);"><span class="required" id="s31">'+""+'</span> Enquiry Canceled Members</div>').appendTo(notificationLists);
					
					$('<div class="notification_header" id="10" onclick="showNotification(this);">Member Daily <br /> Attendance</div>').appendTo(notificationLists);
					$('<div class="notification_header" id="12" onclick="showNotification(this);">Personal Trainer <br/> Daily Attendance</div>').appendTo(notificationLists);

					
					//if(timeAttendence=="true")
					$('<div class="notification_header" id="14" onclick="showNotification(this);">Today Overview</div>').appendTo(notificationLists);
					var isTextSMSModuleGiven='<%=(Boolean)application.getAttribute("isTextSMSModuleGiven")%>';		
					if(isTextSMSModuleGiven=="true"){
						$('<div class="notification_header" id="15" onclick="return false;" style="display:none;">SMS Pack:<span class="required" style="color:rgb(0, 160, 0)"  id="smspack">Active</span> </div>').appendTo(notificationLists);

					}else{	
						$('<div class="notification_header" id="15" onclick="contactToActivate();return false;" style="display:none;">SMS Pack:<span class="required" id="smspack">De-Active</span> </div>').appendTo(notificationLists);

					}
					//=================================	
			
			},
			complete:function(xhr,status){
				//$(document).ajaxComplete(function(){ $("#wait").css("display", "none");$("#mainContainer").prop("onclick",null);});
				var timeAttendence='<%=application.getAttribute("timeAttendence")%>';	
				if(timeAttendence=="true"){
						var auto = setInterval(function ()
						     	{ 				
									/* $.getJSON('refreshEsslStatus',{})			//Json To Refresh Data
									.done(function(data)
									{	$("#essl1").text(data.esslStatus); 
										setStatusColor(data.esslStatus);
									});  */	
									
									$.ajax({url:'refreshEsslStatus',global:false,			//Json To Refresh Data
										success:function(data)
										{	$("#essl1").text(data.esslStatus); 
											setStatusColor(data.esslStatus);
										}}); 
									todayOverView();
									
						     	}, 1000 * 2 ); // refresh every 2 sec
				} else{				
					var auto = setInterval(function (){ todayOverView();}, 1500 ); // refresh every 1.5 sec
				}
				var auto = setInterval(function ()
			    {    	
					    	$.ajax({url:'refreshAllNotificationList',global:false,			//Json To Refresh Data
					    		success:function(data)
							{						
								$("#s1").text(data.pendingFeesList.length);
								$("#s11").text(data.cancelPendingFeesList.length);
								$("#s2").text(data.reNewList.length);
								$("#s3").text(data.birthDayList.length);
								$("#s4").text(data.anniversaryDayList.length);
								$("#s5").text(data.attendanceList.length);
								$("#s6").text(data.appointmentList.length);
								$("#s7").text(data.enquirytList.length);
								$("#s8").text(data.inactiveMemberList.length);
								$("#s9").text(data.activeMemberList.length);
								$("#s20").text(data.cancelMemberList.length);
								$("#s31").text(data.regularFollowUpList.length);	
							}}); 		
			     }, 1000 * 60 * 2); // refresh every 2 minute
				
			}}); 
		
  			   	
				    	$.ajax({url:'refreshAllNotificationList',global:false,			//Json To Refresh Data
				    		success:function(data)
						{						
							$("#s1").text(data.pendingFeesList.length);
							$("#s11").text(data.cancelPendingFeesList.length);
							$("#s2").text(data.reNewList.length);
							$("#s3").text(data.birthDayList.length);
							$("#s4").text(data.anniversaryDayList.length);
							$("#s5").text(data.attendanceList.length);
							$("#s6").text(data.appointmentList.length);
							$("#s7").text(data.enquirytList.length);
							$("#s8").text(data.inactiveMemberList.length);
							$("#s9").text(data.activeMemberList.length);
							$("#s20").text(data.cancelMemberList.length);
							$("#s31").text(data.regularFollowUpList.length);	
							$("#s32").text(data.pendingFeesRegularFollowUpList.length);	
						}}); 
		    
		//Change Pendinding Fees End Date to user format
		$("[id^='dueDate']").show(function(){
			$(this).text("Due Date: "+convertToUserDate($(this).text()));
		});
		//Change Fees End Date to user format
		$("[id^='endDate']").show(function(){
			$(this).text("End Date: "+convertToUserDate($(this).text()));
		});
		//Change Attendance Date to user format 
		$("[id^='attendanceDate']").show(function(){
			$(this).text("Last Attended: "+convertToUserDate($(this).text()));
		});
		
		//Change appointmentDate Date to user format 
		$("[id^='appointmentDate']").show(function(){
			$(this).text("Appointment Date: "+convertToUserDate($(this).text()));
		});
		//Change Birthday Date to user format
		$("[id^='birthDay']").show(function(){
			$(this).text("BirthDay: "+convertToUserDate($(this).text()));
		});  
		//Change AnniversaryDay Date to user format
		$("[id^='anniversaryDay']").show(function(){
			$(this).text("AnniversaryDay: "+convertToUserDate($(this).text()));
		});  
		//Change Reply Date to user format
		$("[id^='replyDate']").show(function(){
			$(this).text("Reply Date: "+convertToUserDate($(this).text()));
		});

		//Check unchake on click
		$(".member").click(function(){
			var check = "#check"+$(this).attr('id');
			if($(check).attr('checked')=='checked'){
				 $(check).removeAttr("checked");
				 $(this).attr("class","member");
			 }else{
				 $(check).attr("checked",'checked');
				 $(this).attr("class","checkedMember");
			 }
		});
		
		//Check unchake on click
		$(".renewBefore").click(function(){			
			var check = "#check"+$(this).attr('id');
			if($(check).attr('checked')=='checked'){
				 $(check).removeAttr("checked");
				 $(this).attr("class","renewBefore");
			 }else{
				 $(check).attr("checked",'checked');
				 $(this).attr("class","checkedMember");
			 }
		});
		//Check unchake on click
		$(".renewAfter").click(function(){
			var check = "#check"+$(this).attr('id');
			if($(check).attr('checked')=='checked'){
				 $(check).removeAttr("checked");
				 $(this).attr("class","renewAfter");
			 }else{
				 $(check).attr("checked",'checked');
				 $(this).attr("class","checkedMember");
			 }
		});			
	
	
	
	
	});
	function contactToActivate(){
			alertMsg("Please! contact to activate SMSPack. Thank You.");
	}
	function setNotificationLists(notificationStatus){			
		  if(notificationStatus=="hide"){				  
		  		$("#toggle").css("background-image","url('<%=request.getContextPath() %>/images/rightToggle.jpg')");		  		
		  }	
		  else{				  
			  $("#toggle").css("background-image","url('<%=request.getContextPath() %>/images/leftToggle.jpg')");
		  }  
		  $("#notificationLists" ).animate({width: notificationStatus},500);
		}
		var fromNotificationClick=false;
		function showNotification(id){
			var divClass = $(id).attr('class');			
			if(divClass == "notification_header"){
				/* var areaId = "#notification_area_"+$(".click_notification_header").attr('id'); */		
				var areaId = "#notification_area_"+$(".notification_header").attr('id');
				$(areaId).hide();
				//$(".click_notification_header").attr('class',"notification_header");
				$(".notification_area").hide();
				//$("#allNotification").hide();				
				$("#notification_area_"+$(id).attr('id')).show('slow');
				//$(this).attr('class','click_notification_header');
				$(".notification_header").css('color','');
				$(id).css('color','blue');
				$("#bodyContainer").hide();
				$(".labelClass").empty();
				$(".areaClass").empty();				
				var id =$(id).attr('id');		
				
				switch(parseInt(id)){
				 case 1:										
					 loadPendingFeesList();
				     break;
				 case 2:
					 loadReNewList();
				        break;
				 case 3:
					 loadBirthDayList();
				        break;
				        
				 case 4:
					 loadAnniversaryDayList();				        
					 break;	 
					 
				 case 5:
					 loadAttendanceList();
				        break;
				 case 6:
					 loadAppointmentList();
				        break;
				 case 7:
					 loadEnquirytList();
				        break;
				 case 8:
					 loadInactiveMemberList();
					 break;
				 case 9:
					 activeMemberList();
					 break;
				 case 10:
					 $("#displayNotificationMemberId").val("");					
					 loadDailyAttendance();
					 NotificationClearForm();
					 //alert("case 10");
					 //onMemberIdChange1();
					 break;
				 case 11:										
					 loadCancelPendingFeesList();
				     break;
				 case 12:
					 $("#displayNotificationPersonalTrainerId").val("");
					 loadDailyPTAttendance();
					 PTNotificationClearForm();
					 break; 
				 case 13:								
					 esslSettingForm();					 
					 break;
				 case 14:								
					// todayOverView();
					fromNotificationClick=true;
					 break;
				 case 20:
					 loadCancelMembersList();
					 break;
				 case 30:
					 showEnquiredMembers();
					 break;
					 
				 case 31:
					 //showEnquiryCanceledMembers();
					 showRegularFollowUps();
					 break;
				 case 32:
					 //showEnquiryCanceledMembers();
					 showRegularPendingFollowUps();
					 break;
					 
				default:
				}
				
			}
			 $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
			 $(".totalFees").css("color","#990099");
		}
		 function closeAction(id){
			$(".notification_header").css('color','');
			$(".notification_area").hide();
			$("#bodyContainer").show();		
		}
		 var isExeComplete=false;
		 function  todayOverView(){ 
			 if(!isExeComplete){
				 isExeComplete=true;
			 $('#label14').empty();
			 $('<u>Today Present Members Detail List</u>').appendTo($('#label14'));
			/*  $.getJSON('fetchTodayPresentMemberDetailList',{})			//Json To Fetch Data
				.done(function(data) */
						$.ajax({url:'fetchTodayPresentMemberDetailList',global:false,			//Json To Refresh Data
				    		success:function(data)
						{		
				    			
							if((data.beepType!="noBeep" || fromNotificationClick) && data.todayOverviewDetailList.length>0){
								//alert(data.beepType);	
								if(data.beepType=="beep1")	{			    			
				    				beep1(); 
				    			}
				    			else if(data.beepType=="beep2"){
				    				beep11();
				    				 
				    			}else{}	
								fromNotificationClick=false;
								var list=data.todayOverviewDetailList;
								//alert(data.todayOverviewDetailList.length);	
								//alert(list.length);	
								var area14=$("#area14");area14.empty();
								var table=$('<table ></table>').appendTo(area14);
								var row =$('<tr></tr>').appendTo(table);
								var i=0;
								for( i=0;i<list.length;i++){	
									
									if(i%3==0)
										 row =$('<tr></tr>').appendTo(table);	
									var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);
									var div=$('<div class="member" style="height: 120px" id="block11'+i+'"></div>').appendTo(td);
									var innerTable=$('<table title="Ledger Name: '+list[i].ledgerName+'"></table>').appendTo(div);
									var memBerStatus="";
									//alert("list[i].isInActive="+list[i].isInActive+" list[i].isPending="+list[i].isPending);
									if(list[i].receiptNo!=null){
									
									if(list[i].isInActive==true && list[i].isPending==true ){										
										memBerStatus="<u>INACTIVE && PENDING </u>";
									}else if(list[i].isInActive==true){										
										memBerStatus="<u>INACTIVE </u>";
									}else if(list[i].isPending==true ) {									
										memBerStatus="<u>PENDING </u>";
									}else{
										memBerStatus="<u>ACTIVE </u>";
									}
									}else{
										memBerStatus="<u><br/>Yet not issued any receipt</u>";
									}
									//alert("memBerStatus="+memBerStatus);
									//$('<tr><td colspan="2">'+memBerStatus+'</td></tr>').appendTo(innerTable);
									 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"	name="check['+list[i].memberId+']" style="display: none;" checked="checked"/>'
									+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td><td id=memberStatus>'+memBerStatus+' </td></tr>').appendTo(innerTable);
									 if(list[i].receiptNo!=null){var tr2=$('<tr><td colspan="2">Name: '+list[i].name+'</td></tr>').appendTo(innerTable);
								 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
									var tr4=$('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name: '+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
									var tr5=$('<tr><td colspan="" id="endDate'+list[i].memberId+'">Start Date : '+convertToUserDate(list[i].startDate)+'&nbsp;</td><td colspan="" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
									//var tr6=$('<tr><td colspan="2" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
									var tr6=$('<tr><td class="totalFees" style="padding-right: 10px;">Total Fees: '+list[i].ledgerTotalFees+'</td><td class="pending">'
									+'Pending: '+list[i].ledgerPendingFees+'</td></tr>').appendTo(innerTable); 	}							
									if(list[i].receiptNo!=null){	
										if(list[i].isInActive==true && list[i].isPending==true ){
											$("#block11"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");	//red	
										}else if(list[i].isInActive==true){
											$("#block11"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");	//red	
										}
										else if(list[i].isPending==true ) {
											$("#block11"+i).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");  //yellow
										}else{
											$("#block11"+i).css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");		//green	
										}	
									}else{
										$("#block11"+i).css("background", "none");		//green	
									}
										
								}				
								// $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
								// $(".totalFees").css("color","#990099");
														
				    				
							}							
							
						},
						complete:function(xhr,status){
							isExeComplete=false;
						}}); 
			 }
			}	 
	 
	 function esslSettingForm(){
		 $('<u>Essl Attendence Setting.</u>').appendTo($('#label13'));
			$.getJSON('fetchEsslSettingAction',{}).done(function(data){
				var  serverName="";
				var databaseName="";
				var  portNo="";
				var esslIP="";
				var list = data.esslDetails;
				if(list==""){
					alert("No Details");
				}else{
					serverName=list[0].serverName;
					databaseName=list[0].databaseName;
					portNo=list[0].portNo;
					esslIP=list[0].esslIP;
					
				}
				
				var area = $("#area13");
				area.empty();			
				var table = $('<table></table>').appendTo(area);
				var row = $('<tr></tr>').appendTo(table);
				var td1 = $('<td>Server name / IP Address</td>').appendTo(row);				
				var td2 = $('<td colspan="3"><input type="text" name="serverName" id="serverName" size="68" value="'+serverName+'"></td>').appendTo(row);
				
				row =$('<tr></tr>').appendTo(table);
				 td1 = $('<td>ESSL IP Address</td>').appendTo(row);				
				 td2 = $('<td colspan="3"><input type="text" name="esslIP" id="esslIP" size="68" value="'+esslIP+'"></td>').appendTo(row);
				
				row = $('<tr></tr>').appendTo(table);
				 td1 = $('<td>Server Port No</td>').appendTo(row);
				 td2 = $('<td><input type="text" name="portNo" id="portNo" value="'+portNo+'"></td>').appendTo(row);
				 var td3 = $('<td>Database Name</td>').appendTo(row);
				 var td4 = $('<td><input type="text" name="databaseName" id="databaseName" value="'+databaseName+'"></td>').appendTo(row);
				
				 row = $('<tr></tr>').appendTo(table);				
				 td1 = $('<td colspan="4"><hr /></td>').appendTo(row);
			
			});
			
			//$(document).trigger('readyAgain1');	
	 }
	  function  loadDailyPTAttendance(){
			
			$('<u>Personal Trainer Daily Attendance</u>').appendTo($('#label12'));
				var area=$("#area12");
				area.empty();		
				
				var div = $('<div style="width: 90%;">').appendTo(area);			
				var fieldset=$('<fieldset><legend>Personal Trainer Daily Attendance</legend>	</fieldset>').appendTo(div);
				var table1=$('<table> </table>').appendTo(fieldset);
				var row1=$('<tr><td><label>Personal Trainer Id</label></td><td><input type="hidden" name="personalTrainerId" id="personalTrainerId" value="" /><input type="hidden" name="attendanceId" id="attendanceId" value="" />'
						+'<input type="text" name="displayPersonalTrainerId" id="displayNotificationPersonalTrainerId" placeholder="" onkeyup="upperCase(this); " tabindex="0" />'
						+'</td>'
						+'<td><label>Personal Trainer Name</label></td>'
						+'<td><input type="text" id="personalTrainerNameAutocompleteNotification" name="personalTrainerNameAutocomplete" placeholder="First Name Middle Name Last Name" style="width:250px;" /></td>'
						+'<td><label>Contact No.</label></td>'
						+'<td valign="top"><input type="text" name="contactNo" id="contactNoNotification"	value="" /></td></tr>').appendTo(table1);
				var hr=$('<hr />').appendTo(fieldset);
				var table2=$('<table class="display" id="attendanceDetails" style="display: none;" border=""></table>').appendTo(fieldset);
				
				var row2 = $('<tr><td rowspan="4">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><label>Personal Trainer Name</label></td><td><label>Date</label></td><td><label>In Time</label></td>	<td><label>Out Time</label></td></tr>').appendTo(table2); 
				var row3 =$('<tr id="tr">'
							+'<td><label id="personalTrainerName"></label></td><td><input type="text" name="attendanceDate" id="attendanceDate" value="'+convertToUserDate(today)+'" readonly="true" style="width:80px;">'
							+'</input></td>'
							+'<td><input type="button" id="in" class="mybtn"  tabindex="1" value="In" class="smallBtn"/>'
							+'<input type="text" name="inTime" id="inTime" readonly="true" value="00:00" style="width:80px; display:none;" /></td>'
							+'<td><input type="button" id="out" class="mybtn"  tabindex="2" value="Out" class="smallBtn"/>'
							+'<input type="text" name="outTime" id="outTime" readonly="true" value="00:00" style="width:80px; display:none;" /></td>'
							+'</tr><tr height="50px;"></tr>').appendTo(table2);		
				
				$(document).trigger('readyAgain1');		
		}
	
	
	function  loadDailyAttendance(){
		$("#scanMessage").hide();			

		$('<u>Members Daily Attendance</u>').appendTo($('#label10'));
			var area=$("#area10");
			area.empty();		
				
			var div = $('<div style="width: 100%;">').appendTo(area);			
			var fieldset=$('<fieldset><legend>Daily Attendance</legend>	</fieldset>').appendTo(div);
			var table1=$('<table> </table>').appendTo(fieldset);
			var row1=$('<tr><td><label>Member Id</label></td><td><input type="hidden" name="memberId" id="memberId" value="" /><input type="hidden" name="attendanceId" id="attendanceId" value="" />'
					+'<input type="text" name="displayMemberId" id="displayNotificationMemberId" placeholder="" onkeyup="upperCase(this); " tabindex="0" />'
					+'</td>'
					+'<td><label>Member Name</label></td>'
					+'<td><input type="text" id="membersNameAutocompleteNotification" name="membersNameAutocomplete" placeholder="First Name Middle Name Last Name" style="width:250px;" /></td>'
					+'<td><label>Contact No.</label></td>'
					+'<td valign="top"><input type="text" name="contactNo" id="contactNoNotification"	value="" /></td></tr>').appendTo(table1);
			var hr=$('<hr />').appendTo(fieldset);
			var table2=$('<table class="display" id="attendanceDetails" style="display: none;" border=""></table>').appendTo(fieldset);
			
			var row2 = $('<tr><td id="image" align="center" rowspan="3"></td><td><label>Member name</label></td><td><label>Date</label></td><td><label>In Time</label></td>	<td><label>Out Time</label></td></tr>').appendTo(table2);
			
			var row3 =$('<tr id="tr">'
						+'<td><label id="memberName"></label></td><td><input type="text" name="attendanceDate" id="attendanceDate" value="'+convertToUserDate(today)+'" readonly="true" style="width:80px;">'
						+'</input></td>'
						+'<td><input type="button" id="in" class="mybtn"  tabindex="1" value="In" class="smallBtn"/>'
						+'<input type="text" name="inTime" id="inTime" readonly="true" value="00:00" style="width:80px; display:none;" /></td>'
						+'<td><input type="button" id="out" class="mybtn"  tabindex="2" value="Out" class="smallBtn"/>'
						+'<input type="text" name="outTime" id="outTime" readonly="true" value="00:00" style="width:80px; display:none;" /></td>'
						/* +'<td><input type="button" id="scanFingr" class="mybtn"  tabindex="3" value="Scan Finger Prints" class="smallBtn"/></td>' */
						/* +'<td><input type="button" id="testFing" class="mybtn"  tabindex="4" value="Test Finger Prints" class="smallBtn"/>'
					 	+'<label id="scanMessage"></label></td>' */
						+'</tr><tr height="50px;"></tr>').appendTo(table2);		
			var fieldset2 = $('<fieldset id="fieldset2" style="display:none;"></fieldset>').appendTo(div);
			
			var table3 = $('<table align="left"></table>').appendTo(fieldset2);
			var row4 =$('<tr><td><label>Joining Date</label></td><td style=" padding-left: 16px;"><label id="joiningDate"></label></td><td style=" padding-left: 16px;"><label>Total Fees</label></td><td><label id="totalFees"></label></td></tr>').appendTo(table3);
			
			var row5 = $('<tr><td><label>Due Date</label></td><td style=" padding-left: 16px;"><label id="dueDate"></label></td><td style=" padding-left: 16px;"><label>Paid Fees</label></td><td><label id="paidFees"></label></td></tr>').appendTo(table3);
			var row6 = $('<tr><td><label>End Date</label></td><td style=" padding-left: 16px;"><label id="endDate"></label></td><td style=" padding-left: 16px;"><label>Pending Fees</label></td><td><label id="pendingFees" ></label></td>'
						+'</tr>').appendTo(table3);			
			$(document).trigger('readyAgain');		
	}
	function loadPendingFeesList(){	
		
		  $.getJSON('fetchpendingFeesList',{})			//Json To Fetch Data
		.done(function(data)
		{			
			$('<u>Pending Fees Member List</u>').appendTo($('#label1'));
			var list=data.pendingFeesList;
			var area=$("#area1");
			area.empty();		
			if(data.pendingFeesList.length>0){
				var table=$('<table ></table>').appendTo(area1);
				var row =$('<tr></tr>').appendTo(table);				
				for(var i=0;i<list.length;i++){	
					
					if(i%3==0)
						 row =$('<tr></tr>').appendTo(table);	
					var td=$('<td style="padding: 5px 10px;"></td>').appendTo(row);
					var div=$('<div class="member" style="height: 160px" id="block1'+i+'"></div>').appendTo(td);
					var innerTable=$('<table  title=" Ledger Name: '+list[i].ledgerName+'"></table>').appendTo(div);
					 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"	name="check['+list[i].memberId+']" />'
					+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td></tr>').appendTo(innerTable);
				 	var tr2=$('<tr><td colspan="2">Name: '+list[i].name+'</td></tr>').appendTo(innerTable);
				 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
					var tr4=$('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name: '+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
					//var tr5=$('<tr><td colspan="2" id="dueDate'+list[i].memberId+'">Due Date : '+convertToUserDate(list[i].dueDate)+'&nbsp;</td></tr>').appendTo(innerTable);
					//var tr55=$('<tr><td colspan="2" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);

					var tr5=$('<tr><td colspan="2" id="dueDate'+list[i].memberId+'">Due Date : '+convertToUserDate(list[i].dueDate)+'&nbsp;</td></tr>').appendTo(innerTable);
					 $('<tr><td colspan="2" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
					var tr6=$('<tr><td class="totalFees" style="padding-right: 10px;">Total Fees: '+list[i].ledgerTotalFees+'</td><td class="pending">'
					+'Pending: '+list[i].ledgerPendingFees+'</td></tr>').appendTo(innerTable); 
					
					var tr5=$('<tr><td colspan=""><button style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;" onclick="openModel('+list[i].memberId+',\''+list[i].name+'\')" type="button">Get Follow Up</button></td></tr>').appendTo(innerTable); 
					
					var tr7=$('<tr height="50"></tr>').appendTo(innerTable);	
						if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) < 0 ){
							$("#block1"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");						
					}						
						if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) > 5){
							$("#block1"+i).css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");						
					}						
					if(( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) <= 5 && ( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) >= 0){
						$("#block1"+i).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");
					}
					/*  else
						$("#block1"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");  */
				}				
				// $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
				// $(".totalFees").css("color","#990099");
			}			
			
		});  
		
	}
	
	function loadCancelPendingFeesList(){		
		  $.getJSON('fetchCancelPendingFeesList',{})			//Json To Fetch Data
		.done(function(data)
		{				
			$('<u>Canceled Members Fees Detail List</u>').appendTo($('#label11'));
			var list=data.cancelPendingFeesList;
			var area=$("#area11");
			area.empty();		
			if(data.cancelPendingFeesList.length>0){
				var table=$('<table ></table>').appendTo(area11);
				var row =$('<tr></tr>').appendTo(table);				
				for(var i=0;i<list.length;i++){	
					
					if(i%3==0)
						 row =$('<tr></tr>').appendTo(table);	
					var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);
					var div=$('<div class="member" style="height: 120px" id="block11'+i+'"></div>').appendTo(td);
					var innerTable=$('<table title="Ledger Name: '+list[i].ledgerName+'"></table>').appendTo(div);
					 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"	name="check['+list[i].memberId+']" />'
					+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td></tr>').appendTo(innerTable);
				 	var tr2=$('<tr><td colspan="2">Name: '+list[i].name+'</td></tr>').appendTo(innerTable);
				 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
					var tr4=$('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name: '+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
					var tr5=$('<tr><td colspan="" id="dueDate'+list[i].memberId+'">Due Date : '+convertToUserDate(list[i].dueDate)+'&nbsp;</td><td colspan="" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
					//var tr6=$('<tr><td colspan="2" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
					var tr6=$('<tr><td class="totalFees" style="padding-right: 10px;">Total Fees: '+list[i].ledgerTotalFees+'</td><td class="pending">'
					+'Pending: '+list[i].ledgerPendingFees+'</td></tr>').appendTo(innerTable); 
						var tr7=$('<tr height="50"></tr>').appendTo(innerTable);
						//alert((  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday));
				  if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) < 0 ){
							$("#block11"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");						
					}else 						
				  if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) > 5){
							$("#block11"+i).css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");						
					} else						
					if(( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) <= 5 && ( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) >= 0){
						$("#block11"+i).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");
					}
					/*  else
						$("#block1"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");  */
				}				
				// $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
				// $(".totalFees").css("color","#990099");
			}			
			
		});  
		
	}
	function loadBirthDayList(){	
		$('#label3').empty();
		$('<input id="hiddenFromDate" type="hidden" /><input id="hiddenToDate" type="hidden" /><span class="required" id="BODCount"></span>&nbsp;<u>Birth Days.</u> From <a id="fromBD">'+convertToUserDate(today)+'</a> To <a id="toBD">'+convertToUserDate(today)+'</a>').appendTo($('#label3'));
		$('#label3').append('<div></div>');			
		loadBDList(false);		 
		$(document).trigger('readyAgain');	
	}
	function loadReNewList(){	
		$('#label2').empty();
		$('<input id="hiddenFromReNewDate" type="hidden" /><input id="hiddenToReNewDate" type="hidden" /><span class="required" id="ReNewCount"></span>&nbsp;<u>Members Detail By Package End Date.</u> From <a id="fromReNewD">'+convertToUserDate(today)+'</a> To <a id="toReNewD">'+convertToUserDate(today)+'</a>').appendTo($('#label2'));
		$('#label2').append('<div></div>');			
		loadReNewPackageList(false);		 
		$(document).trigger('readyAgain');	
	}
	function loadReNewPackageList(fetchAgain){	
		var fromDate=$('#fromReNewD').text(); var toDate=$('#toReNewD').text();		
		  $.getJSON('fecthreNewList',{fromDate:fromDate,toDate:toDate,fetchAgain:fetchAgain})			//Json To Fetch Data
		.done(function(data)
		{	
			//$('<u>Members Detail List By Package End Date.</u>').appendTo($('#label2'));
			var list=data.reNewList;
			var area=$("#area2");$("#ReNewCount").text(list.length);			
			area.empty();			
			if(list.length>0){
				var table=$('<table ></table>').appendTo(area);
				var row =$('<tr></tr>').appendTo(table);				
				for(var i=0;i<list.length;i++){	
					if(i%3==0)
						 row =$('<tr></tr>').appendTo(table);	
					var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);					
					var div=$('<div class="member" style="height: 120px" id="block2'+i+'"></div>').appendTo(td);
					var innerTable=$('<table  title="Re-New Package List:'+list[i].ledgerName+'"></table>').appendTo(div); 
					 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"	name="check['+list[i].memberId+']" />'
					+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td></tr>').appendTo(innerTable);
					var tr2=$('<tr><td colspan="2">Name: '+list[i].name+'</td></tr>').appendTo(innerTable);
					var tr3=$('<tr><td colspan="2" id="endDate'+list[i].endDate+'">End Date:'+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
				 	var tr4=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
					var tr5=$('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Re-New Package List: '+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);					
					var tr6=$('<tr height="50"></tr>').appendTo(innerTable); 
				  if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) < 0 ){
							$("#block2"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");	//red					
					}						
				  if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) == 2){
							$("#block2"+i).css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");	//green					
					}						
					if( ( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) < 2 && ( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) >= 0){
						$("#block2"+i).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");  //Yellow
					}
				}				
				/*  $(".member").css("background","linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)"); */
				
				//$(".member").width(screen.availWidth*0.18);
				
			}			
			
		});  
		
	}
	function loadBDList(fetchAgain){
		var fromDate=$('#fromBD').text(); var toDate=$('#toBD').text();		
		 $.getJSON('fecthbirthDayList',{fromDate:fromDate,toDate:toDate,fetchAgain:fetchAgain})			//Json To Fetch Data
			.done(function(data)
			{				
				
				var list=data.birthDayList;
				$("#BODCount").text(list.length);
				var area=$("#area3");
				area.empty();
				if(list.length>0){
					var table=$('<table ></table>').appendTo(area);
					var row =$('<tr></tr>').appendTo(table);				
					for(var i=0;i<list.length;i++){	
						if(i%3==0)
							 row =$('<tr></tr>').appendTo(table);	
						var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);					
						var div=$('<div class="member" style="height: 120px" id="'+list[i].memberId+'"></div>').appendTo(td);
						var innerTable=$('<table ></table>').appendTo(div);
						 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"name="check['+list[i].memberId+']" style="display: none;" checked="checked" />'
						+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td></tr>').appendTo(innerTable);
						var tr2=$('<tr><td colspan="2">Name: '+list[i].firstName+' '+list[i].middleName+' '+list[i].lastName+'</td></tr>').appendTo(innerTable);				
					 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
						var tr4=$('<tr><td colspan="2">'+convertToUserDate(list[i].dateOfBirth)+'</td></tr>').appendTo(innerTable);					
						var tr5=$('<tr height="50"></tr>').appendTo(innerTable); 	 
					}				
					 $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
					 $(".totalFees").css("color","#990099");
				}			
				
			}); 			  
	}
	function loadBirthDayList(){	
		$('#label3').empty();
		$('<input id="hiddenFromDate" type="hidden" /><input id="hiddenToDate" type="hidden" /><span class="required" id="BODCount"></span>&nbsp;<u>Birth Days.</u> From <a id="fromBD">'+convertToUserDate(today)+'</a> To <a id="toBD">'+convertToUserDate(today)+'</a>').appendTo($('#label3'));
		$('#label3').append('<div></div>');			
		loadBDList(false);		 
		$(document).trigger('readyAgain');	
	}	
	function loadAnniversaryDayList(){	
		$('#label4').empty();
		$('<input id="hiddenFromAnDate" type="hidden" /><input id="hiddenToAnDate" type="hidden" /><u>Todays&nbsp;</u><span class="required" id="AnDCount"></span>&nbsp;<u>Wedding Anniversaries.</u> From <a id="fromAnD">'+convertToUserDate(today)+'</a> To <a id="toAnD">'+convertToUserDate(today)+'</a>').appendTo($('#label4'));
		$('#label4').append('<div></div>');			
		loadAnList(false);		 
		$(document).trigger('readyAgain');	
	}
	
	function loadAnList(fetchAgain){		
		var fromDate=$('#fromAnD').text(); var toDate=$('#toAnD').text();	
		  $.getJSON('fecthanniversaryDayList',{fromDate:fromDate,toDate:toDate,fetchAgain:fetchAgain})			//Json To Fetch Data
			.done(function(data)
			{	
				//$('<u>Todays Anniversary Day.</u>').appendTo($('#label4'));
				var list=data.anniversaryDayList;
				$("#AnDCount").text(list.length);
				var area=$("#area4");
				area.empty();				
				if(list.length>0){
					var table=$('<table ></table>').appendTo(area);
					var row =$('<tr></tr>').appendTo(table);			
					for(var i=0;i<list.length;i++){							
						if(i%3==0)
						 row =$('<tr></tr>').appendTo(table);					
						var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);					
						var div=$('<div class="member" style="height: 120px" id="'+list[i].memberId+'"></div>').appendTo(td);
						var innerTable=$('<table ></table>').appendTo(div);
						 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"	name="check['+list[i].memberId+']" style="display: none;" checked="checked"/>'
						+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td></tr>').appendTo(innerTable);
						var tr2=$('<tr><td colspan="2">Name: '+list[i].firstName+' '+list[i].middleName+' '+list[i].lastName+'</td></tr>').appendTo(innerTable);				
					 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
						var tr4=$('<tr><td colspan="2">'+convertToUserDate(list[i].anniversaryDate)+'</td></tr>').appendTo(innerTable);				
						var tr5=$('<tr height="50"></tr>').appendTo(innerTable); 	 
					}				
					 $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
					 $(".totalFees").css("color","#990099");
				}			
				
			});  
		  
	}
	
	function loadAttendanceList(){		
		  $.getJSON('fecthattendanceList',{})			//Json To Fetch Data
			.done(function(data)
			{	
				$('<u>Member List Absent From Four or More Than Four Days.</u>').appendTo($('#label5'));
				var list=data.attendanceList;
				var area=$("#area5");
				area.empty();				
				if(list.length>0){
					var table=$('<table ></table>').appendTo(area);
					var row =$('<tr></tr>').appendTo(table);				
					for(var i=0;i<list.length;i++){	
						if(i%3==0)
							 row =$('<tr></tr>').appendTo(table);	
						var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);					
						var div=$('<div class="member" style="height: 120px" id="'+list[i].memberId+'"></div>').appendTo(td);
						var innerTable=$('<table ></table>').appendTo(div);
						 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"	name="check['+list[i].memberId+']" style="display: none;" checked="checked"/>'
						+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td></tr>').appendTo(innerTable);
						var tr2=$('<tr><td colspan="2">Name: '+list[i].firstName+' '+list[i].middleName+' '+list[i].lastName+'</td></tr>').appendTo(innerTable);				
					 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
						var tr4=$('<tr><td colspan="2">Last Presenty:'+convertToUserDate(list[i].attendanceDate)+'</td></tr>').appendTo(innerTable);					
						var tr5=$('<tr height="50"></tr>').appendTo(innerTable); 	 
					}				
					 $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
					 $(".totalFees").css("color","#990099");
				}			
				
			});  
		  
	}
	function loadAppointmentList(){		
		  $.getJSON('fecthappointmentList',{})			//Json To Fetch Data
			.done(function(data)
			{	$('<u>Todays Appointment.</u>').appendTo($('#label6'));			
				var list=data.appointmentList;
				var area=$("#area6");
				area.empty();				
				if(list.length>0){
					var table=$('<table ></table>').appendTo(area);
					var row =$('<tr></tr>').appendTo(table);				
					for(var i=0;i<list.length;i++){	
						if(i%3==0)
							 row =$('<tr></tr>').appendTo(table);	
						var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);					
						var div=$('<div class="member" style="height: 120px" id="'+i+'"></div>').appendTo(td);
						var innerTable=$('<table ></table>').appendTo(div);
						 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+i+'"	name="check['+i+']" /></td></tr>').appendTo(innerTable);
								 
							var tr2=$('<tr><td colspan="2">Name: '+list[i].firstNameAppointment+' '+list[i].lastNameAppointment+'</td></tr>').appendTo(innerTable);				
					 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNoAppointment+'</td></tr>').appendTo(innerTable);
						var tr4=$('<tr><td colspan="2">'+convertToUserDate(list[i].appointmentDate)+'</td></tr>').appendTo(innerTable);	
						var tr5=$('<tr><td colspan="2">Time: '+list[i].appointmentTime+'</td></tr>').appendTo(innerTable);	
						var tr6=$('<tr height="50"></tr>').appendTo(innerTable); 	 
					}				
					 $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
					 $(".totalFees").css("color","#990099");
				}			
				
			});  
		  
	}
	
	function loadEnquirytList(){		
		  $.getJSON('fecthenquirytList',{})			//Json To Fetch Data
			.done(function(data)
			{	
				console.log(data);
				$('<u>Todays Follow Up List.</u>').appendTo($('#label7'));
			
				var list=data.enquirytList;
				var area=$("#area7");
				area.empty();			
				if(list.length>0){
					var table=$('<table ></table>').appendTo(area);
					var row =$('<tr></tr>').appendTo(table);				
					for(var i=0;i<list.length;i++){							
						if(i%3==0)
							 row =$('<tr></tr>').appendTo(table);	
						var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);					
						var div=$('<div class="member" style="height: 120px" id="'+list[i].displayEnquiryId+'"></div>').appendTo(td);
						var innerTable=$('<table ></table>').appendTo(div);
						 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].displayEnquiryId+'"name="check['+list[i].displayEnquiryId+']" /></td></tr>').appendTo(innerTable);

						var tr2=$('<tr><td colspan="2">Name: '+list[i].firstNameEnquiry+' '+list[i].lastNameEnquiry+'</td></tr>').appendTo(innerTable);				
					 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNoEnquiry+'</td></tr>').appendTo(innerTable);						
					 	var tr4=$('<tr><td colspan="2">Enquiry Handled By: '+list[i].enquiryHandledBy+'</td></tr>').appendTo(innerTable);	
					 	var tr5=$('<tr><td colspan="2">'+convertToUserDate(list[i].replyDate)+'</td></tr>').appendTo(innerTable);
					 	var tr7=$('<tr><td colspan="2"><a href="DisplayWholeRecord?contact='+list[i].contactNoEnquiry+'" target="_blank">Display</td></tr>').appendTo(innerTable);
					 	var tr9=$('<tr><td colspan="2"><a target="_blank" href="cancelEnquiry?enqId='+list[i].displayEnquiryId+'" style="color:red;" >Cancel</a>&nbsp;&nbsp;</td><td colspan="2"><a href="enquiryUpdate?enqId='+list[i].displayEnquiryId+'" target="_blank">Re-Enquiry</td></tr>').appendTo(innerTable);
					 	
						var tr6=$('<tr height="50"></tr>').appendTo(innerTable); 	 
					}				
					 $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
					 $(".totalFees").css("color","#990099");
				}			
				
			});  
		  
					
	}
	function showEnquiredMembers()
	{
		$.getJSON('fetchInEnquiredMembers',{}).done(function(data){
			
			console.log(data);
			$('<u>Todays Follow Up List.</u>').appendTo($('#label30'));
		
			var list=data.enquiredMemberList;
			var area=$("#area30");
			area.empty();			
			if(list.length>0){
				var table=$('<table ></table>').appendTo(area);
				var row =$('<tr></tr>').appendTo(table);				
				for(var i=0;i<list.length;i++){							
					if(i%3==0)
						 row =$('<tr></tr>').appendTo(table);	
					var td=$('<td style="padding: 5px 5px;"></td>').appendTo(row);					
					var div=$('<div class="member" style="height: 130px" id="'+list[i].displayEnquiryId+'"></div>').appendTo(td);
					var innerTable=$('<table ></table>').appendTo(div);
					 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].displayEnquiryId+'"name="check['+list[i].displayEnquiryId+']" /></td></tr>').appendTo(innerTable);
   
					var tr2=$('<tr><td colspan="2">Name: '+list[i].firstNameEnquiry+' '+list[i].lastNameEnquiry+'</td></tr>').appendTo(innerTable);				
				 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNoEnquiry+'</td></tr>').appendTo(innerTable);						
				 	var tr4=$('<tr><td colspan="2">Enquiry Handled By: '+list[i].enquiryHandledBy+'</td></tr>').appendTo(innerTable);	
				 	var tr5=$('<tr><td colspan="2">'+convertToUserDate(list[i].replyDate)+'</td></tr>').appendTo(innerTable);
				 	var tr6=$('<tr><td colspan="2">feedback: '+list[i].feedbackEnquiry+'</td></tr>').appendTo(innerTable);
				 	var tr7=$('<tr><td colspan="2"><a href="DisplayWholeRecord?contact='+list[i].contactNoEnquiry+'" target="_blank">Display</td></tr>').appendTo(innerTable);
				 	var tr9=$('<tr><td colspan="2"><a href="enquiryUpdate?enqId='+list[i].displayEnquiryId+'" target="_blank">Re-Enquiry</td><td colspan="2">&nbsp;&nbsp<a target="_blank" href="cancelEnquiry?enqId='+list[i].displayEnquiryId+'" style="color:red;" >Cancel</a></td></tr>').appendTo(innerTable);
				 	 
				 	var tr8=$('<tr height="50"></tr>').appendTo(innerTable); 	 
				}				
				 $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
				 $(".totalFees").css("color","#990099");
			}			
		});
			
		
	} 
	function showRegularFollowUps(){
	 
		$.getJSON('fetchRegularFollowUp',{}).done(function(data){
			$('<u>Active Members Package List.</u>').appendTo($('#label31'));
			var list = data.regularFollowUpList;
			console.log(data);
			var area = $("#area31");
			area.empty();
			if(list.length>0){
				
				var table = $('<table ></table>').appendTo(area);
				var row = $('<tr></tr>').appendTo(table);
				for(var i=0; i<list.length;i++){
					if(i%3==0)
						row = $('<tr></tr>').appendTo(table);					
					
					var td = $('<td style="padding: 5px 5px;"></td>').appendTo(row);
					var div = $('<div class="member" style="height: 120px" id="block'+i+'" value="act"></div>').appendTo(td);
					var innerTable = $('<table title="Ledger Name:'+list[i].ledgerName+'"></table>').appendTo(div);
					var tr1 = $('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'" name="check['+list[i].memberId+']" value="'+list[i].memberId+'" />'
						+'<img src="<%=request.getContextPath()%>/MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+'<td></tr>').appendTo(innerTable);
					
					var tr2 = $('<tr><td colspan="2">Name: '+list[i].firstName+' '+list[i].middleName+' '+list[i].lastName+'<td><tr>').appendTo(innerTable);
					var tr3 = $('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
					var tr4 = $('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name:'+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
					var tr5 = $('<tr><td colspan="2">End Date: '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
					
					$('<tr><td colspan=""><button style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;" onclick="openModelRegMembers('+list[i].memberId+',\''+list[i].firstName+'\')" type="button">Feedback</button></td><td><a class="btn btn-info" href="searchMember?memberId='+list[i].memberId+'">GetInfo</a></td></tr>').appendTo(innerTable); 
					var tr6 = $('<tr height="50"></tr>').appendTo(innerTable);
					if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) > 5){
							$("#block"+i).css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");						
					}						
					else{
						$("#block"+i).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");
					}				
		
				}
			}
		});
	}
	function showRegularPendingFollowUps(){
		 $.getJSON('fetchRegularPendingFeeFollowUp',{})			//Json To Fetch Data
			.done(function(data)
			{	console.log("---------");		console.log(data);
				$('<u>Pending Follow Up</u>').appendTo($('#label32'));
				var list=data.pendingFeesRegularFollowUpList;
				var area=$("#area32");
				area.empty();		
				if(data.pendingFeesRegularFollowUpList.length>0){
					var table=$('<table ></table>').appendTo(area);
					var row =$('<tr></tr>').appendTo(table);				
					for(var i=0;i<list.length;i++){	
						
						if(i%3==0)
							 row =$('<tr></tr>').appendTo(table);	
						var td=$('<td style="padding: 5px 10px;"></td>').appendTo(row);
						var div=$('<div class="member" style="height: 160px" id="block1'+i+'"></div>').appendTo(td);
						var innerTable=$('<table  title=" Ledger Name: '+list[i].ledgerName+'"></table>').appendTo(div);
						 var tr1=$('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'"	name="check['+list[i].memberId+']" />'
						+'<img src="MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+' </td></tr>').appendTo(innerTable);
					 	var tr2=$('<tr><td colspan="2">Name: '+list[i].name+'</td></tr>').appendTo(innerTable);
					 	var tr3=$('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
						var tr4=$('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name: '+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
						//var tr5=$('<tr><td colspan="2" id="dueDate'+list[i].memberId+'">Due Date : '+convertToUserDate(list[i].dueDate)+'&nbsp;</td></tr>').appendTo(innerTable);
						//var tr55=$('<tr><td colspan="2" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);

						var tr5=$('<tr><td colspan="2" id="dueDate'+list[i].memberId+'">Due Date : '+convertToUserDate(list[i].dueDate)+'&nbsp;</td></tr>').appendTo(innerTable);
						 $('<tr><td colspan="2" id="endDate'+list[i].memberId+'">End Date : '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
						var tr6=$('<tr><td class="totalFees" style="padding-right: 10px;">Total Fees: '+list[i].ledgerTotalFees+'</td><td class="pending">'
						+'Pending: '+list[i].ledgerPendingFees+'</td></tr>').appendTo(innerTable); 
						
						var tr5=$('<tr><td colspan=""><button style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;" onclick="openModelRegFllPendingFees('+list[i].memberId+',\''+list[i].name+'\')" type="button">Get Follow Up</button></td></tr>').appendTo(innerTable); 
						
						var tr7=$('<tr height="50"></tr>').appendTo(innerTable);	
							if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) < 0 ){
								$("#block1"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");						
						}						
							if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) > 5){
								$("#block1"+i).css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");						
						}						
						if(( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) <= 5 && ( (new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) >= 0){
							$("#block1"+i).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");
						}
						/*  else
							$("#block1"+i).css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");  */
					}				
					// $(".pending").css("color","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
					// $(".totalFees").css("color","#990099");
				}			
				
			}); 
	}
	
	function loadInactiveMemberList(){
		$.getJSON('fetchInactiveMemberList',{}).done(function(data){
			$('<u>Inactive Members Package List.</u>').appendTo($('#label8'));
			var list = data.inactiveMemberList;
			var area = $("#area8");
			area.empty();
			
			if(list.length>0){
				
				var table = $('<table></table>').appendTo(area);
				var row = $('<tr></tr>').appendTo(table);
				for(var i=0; i<list.length;i++){
					if(i%3==0)
						row = $('<tr></tr>').appendTo(table);
					var ctx ='<%=request.getContextPath()%>';
					var td = $('<td style="padding: 5px 5px;"></td>').appendTo(row);				
					var div = $('<div class="member" style="height: 120px" id="'+list[i].memberId+'"></div>').appendTo(td);
					var innerTable = $('<table title="Ledger Name: '+list[i].ledgerName+'"></table>').appendTo(div);
					var tr1 = $('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'" name="check['+list[i].memberId+']" />'
						+'<img src="'+ctx+'/MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+'<td></tr>').appendTo(innerTable);
					
					var tr2 = $('<tr><td colspan="2">Name: '+list[i].firstName+' '+list[i].middleName+' '+list[i].lastName+'<td><tr>').appendTo(innerTable);
					var tr3 = $('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
					var tr4 = $('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name: '+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
					var tr5 = $('<tr><td colspan="2">End Date: '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
					var tr6 = $('<tr height="50"></tr>').appendTo(innerTable);
					
					
				}
				$(".member").css("background","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
			}
		});
		
	}
	
	
	function activeMemberList(){
		
		
	
		$.getJSON('fetchActiveMemberList',{}).done(function(data){
			$('<u>Active Members Package List.</u>').appendTo($('#label9'));
			var list = data.activeMemberList;
			
			var area = $("#area9");
			area.empty();
			if(list.length>0){
				
				var table = $('<table ></table>').appendTo(area);
				var row = $('<tr></tr>').appendTo(table);
				for(var i=0; i<list.length;i++){
					if(i%3==0)
						row = $('<tr></tr>').appendTo(table);					
					
					var td = $('<td style="padding: 5px 5px;"></td>').appendTo(row);
					var div = $('<div class="member" style="height: 120px" id="block'+i+'" value="act"></div>').appendTo(td);
					var innerTable = $('<table title="Ledger Name:'+list[i].ledgerName+'"></table>').appendTo(div);
					var tr1 = $('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'" name="check['+list[i].memberId+']" value="'+list[i].memberId+'" />'
						+'<img src="<%=request.getContextPath()%>/MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+'<td></tr>').appendTo(innerTable);
					
					var tr2 = $('<tr><td colspan="2">Name: '+list[i].firstName+' '+list[i].middleName+' '+list[i].lastName+'<td><tr>').appendTo(innerTable);
					var tr3 = $('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
					var tr4 = $('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name:'+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
					var tr5 = $('<tr><td colspan="2">End Date: '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
					
					$('<tr><td colspan=""><button style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;" onclick="openModelAct('+list[i].memberId+',\''+list[i].firstName+'\')" type="button">Feedback</button></td><td><a class="btn btn-info" href="searchMember?memberId='+list[i].memberId+'">GetInfo</a></td></tr>').appendTo(innerTable); 
					var tr6 = $('<tr height="50"></tr>').appendTo(innerTable);
					if(( (  new Date(list[i].endDate).getTime() - new Date(today).getTime())/(oneday)) > 5){
							$("#block"+i).css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");						
					}						
					else{
						$("#block"+i).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");
					}				
		
				}
				
// 				$(".member").css("background","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
				/*  for(var i=0; i<list.length;i++){
						if(today < list[i].endDate){
							//alert(convertToUserDate(today));
							if(( (new Date(convertToUserDate(today)).getTime() - new Date(convertToUserDate(list[i].endDate)).getTime())/(oneday)) > 5){
							    alert("Green");
								$().css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");
						// $('.member active').attr('list[i].memberId').css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))");
							} else{
								// alert("Yellow");
								//$("#"+list[i].memberId).css("background","linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)"); 
								alert("Yellow");
								 $(this).css("background", "linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");
							}
						}
				}  */
			}
		});
		
	}
	
	function loadCancelMembersList(){
		
		$.getJSON('fetchcancelMemberList',{}).done(function(data){
			$('<u>Cancel Members List.</u>').appendTo($('#label20'));
			var list = data.cancelMemberList;
			
			var area = $("#area20");
			area.empty();
		
			if(list.length>0){
				
				var table = $('<table></table>').appendTo(area);
				var row = $('<tr></tr>').appendTo(table);
				for(var i=0; i<list.length;i++){
					if(i%3==0)
						row = $('<tr></tr>').appendTo(table);
					var ctx ='<%=request.getContextPath()%>';
					var td = $('<td style="padding: 5px 5px;"></td>').appendTo(row);				
					var div = $('<div class="member" style="height: 120px" id="'+list[i].memberId+'"></div>').appendTo(td);
					var innerTable = $('<table title="Ledger Name: '+list[i].ledgerName+'"></table>').appendTo(div);
					var tr1 = $('<tr><td rowspan="6"><input type="checkbox" id="'+list[i].memberId+'" name="check['+list[i].memberId+']" />'
						+'<img src="'+ctx+'/MemberImages/'+list[i].memberId+'.jpg" width="80" height="90" alt="Image" /></td><td id=memberId>Member ID: M'+list[i].memberId+'<td></tr>').appendTo(innerTable);
					
					var tr2 = $('<tr><td colspan="2">Name: '+list[i].firstName+' '+list[i].middleName+' '+list[i].lastName+'<td><tr>').appendTo(innerTable);
					var tr3 = $('<tr><td colspan="2">Contact No: '+list[i].contactNo+'</td></tr>').appendTo(innerTable);
					var tr4 = $('<tr><td colspan="2">'+list[i].ledgerName.split(",").length+' Ledger Name: '+list[i].ledgerName.split(",").reverse().slice(0,1)+'..</td></tr>').appendTo(innerTable);
					var tr5 = $('<tr><td colspan="2">End Date: '+convertToUserDate(list[i].endDate)+'</td></tr>').appendTo(innerTable);
					var tr6 = $('<tr height="50"></tr>').appendTo(innerTable);
					
					$(".member").css("background", "linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");
				}
				

			}
		});
	
	}
	//document.getElementById("attendanceDate").value = Date().toString();
	//document.getElementById("attendanceDate").value = Date().toString(); 

	
</script>
<style type="text/css">
.topClass{
	margin: 0;
}
.labelClass{
font-size: 15px;
font-family: Verdana;
}
#notificationLists {
	width:160px;
    height: 450px;
    margin-left: 5px;
    float: left;
    overflow: auto;
    overflow-x: hidden;
    
}
 #toggle {
  /*   width: 100px;
    height: 100px;
    background: #ccc; */ 
  }
  div.picture1 {
   width:15px;
   height:300px;  /*height of your image*/
  <%--  background-image:url('<%=request.getContextPath() %>/images/leftToggle.jpg') ; --%> /* url('yourimage.file'); */
   margin: 50px 0px 0px 0px;  /* If you want no margin */
   padding:0;
   text-align: center; 
}

</style>

<!-- Body starts here -->

<!-- <div id="dialog" title="Notification"> -->
	<!-- <div id="errorContainer"></div> -->
	
<div class="notification" id="notificationField">

<div class="navbar-inner" style="background:none;">
	<div class="span"  style="position: fixed; left: 0px;  margin-top: 28px;">     <!--  width: 120px;"> -->
	
		<div class="row">		    
			 <div id="toggle" class="picture1 span1"></div>
			<div id="notificationLists" class="span2"></div> 
		</div>
	
	</div>
	
	 <div class="span12"  style="width: 83%; margin-left: 180px; margin-top: 10px;"> 
	<!--  <hr> -->
	<!-- <div  style="padding-top:50px;margin-left:188px; padding-top: 24px;width: 80%;"> -->
	 
	<!-- <div  class="notification_area" id="notification_area_1" style="display: inline;"> -->
	 <div  class="notification_area" id="notification_area_1">
					 
	 <button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
	 	
		<s:form theme="css_xhtml" id="form1">		
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_1');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label1"></label></td>
				</tr>
				
				<div id="myModal" class="modal">
						
						  <!-- Modal content -->
						  <div class="modal-content">
						    <span class="close" onclick="closeModel()">&times;</span>
						   
						   			<div >
							            <form  class="form">
							                <legend style="text-align: center;color:white">Follow Up</legend>
							             
							                
							                        <input type="hidden" name="memberId" id="memberId" value="0">
							                    
							                        <label for="member_name" style="color:white">Member Name  </label>:  <input type="text" name="memberName" class="input-border" id="memberName">
							                    
							                    
							                        <label for="reply_date" style="color:white">Replay Date  </label>: <input type="date" name="replyDate" class="input-border" id="replyDate">
							                    
							                    
							                        <label for="discussion" style="color:white">Discussion  </label>: <textarea name="discussion" class="input-border" id="discussion" cols="25" rows="3"></textarea>
							                
							                    
							                        <label for="called_by" style="color:white">Called By  </label>: <input type="text" name="calledBy" value="" class="input-border" id="calledBy"> 
							                    
							                    	<input type="button" value="submit" onclick="postDiscussioValues()" 
							                    	style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;"
							                    	/>
							            </form>
							            
							        </div>
													   			
						  </div>
						
						</div>
		</table>
				<div id="area1" class="areaClass">
						 
												
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
	 <div class="notification_area" id="notification_area_2">
	 <button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form2">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_2');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label2"></label></td>
				</tr>
		</table>		<div id="area2" class="areaClass">
				
				</div>
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="Your gym membership will expire on date so please pay your remaining fees as soon as possible. Thanks and regards OMVSAB IT Solution "/></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						</td>
						
					</tr>
				</table>			
		</s:form>
	</div>
	
	
	<div class="notification_area" id="notification_area_3">
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form3">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_3');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label3"></label></td>
				</tr>
		</table>
			<div id="area3" class="areaClass">
				
			</div>			
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="Dear Member, Wish you a Very Happy Birthday and a Great Year ahead ! ! Thanks And Regards"/></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
 <div class="notification_area" id="notification_area_4">
 <button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form4">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_4');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label4"></label></td>
				</tr>
		</table>
		
		<div id="area4" class="areaClass">
				
		</div>			
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="Dear Member, Happy Anniversary! May you celebrate many more years together, withstanding the tests of time to emerge stronger and happier ! Thanks and regards"/></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn" />
						<td>
					</tr>
				</table>
			
			
		</s:form>
	</div>
	
	 <div class="notification_area" id="notification_area_5">
	 <button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form5">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_5');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label5"></label></td>
				</tr>
		</table>
		
		<div id="area5" class="areaClass">
				
		</div>			
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn" />
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
	 <div class="notification_area" id="notification_area_6">
	 <button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form6">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_6');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label6"></label></td>
				</tr>
		</table>
		
		<div id="area6" class="areaClass">
				
		</div>	
			
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn" />
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
		 <div class="notification_area" id="notification_area_7">
		 <button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form7">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_7');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label7"></label></td>
				</tr>
		</table>
		
		<div id="area7" class="areaClass">
				
		</div>			
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMSToEnquiredMembers" value="Send" cssClass="smallBtn" />
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>	
	<div  class="notification_area" id="notification_area_8">
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form8">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_8');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label8"></label></td>
				</tr>
		</table>
				<div id="area8" class="areaClass">
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
	<div  class="notification_area" id="notification_area_9">
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form9">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_9');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label9"></label></td>
				</tr>
				<div id="myModalForActiveUsers" class="modal">
						
						  <!-- Modal content -->
						  <div class="modal-content">
						    <span class="close" onclick="closeModelAct()">&times;</span>
						   
						   			<div >
							            <form  class="form">
							                <legend style="text-align: center;color:white">Follow Up</legend>
							             
							                
							                        <input type="hidden" name="memberIdAct" id="memberIdAct" value="0">
							                    
							                        <label for="member_name" style="color:white">Member Name  </label>:  <input type="text" name="memberNameAct" class="input-border" id="memberNameAct">
							                    
							                    
							                        <label for="reply_date" style="color:white">Replay Date  </label>: <input type="date" name="replyDateAct" class="input-border" id="replyDateAct">
							                    
							                    
							                        <label for="discussion" style="color:white">Discussion  </label>: <textarea name="discussionAct" class="input-border" id="discussionAct" cols="25" rows="3"></textarea>
							                
							                    
							                        <label for="called_by" style="color:white">Called By  </label>: <input type="text" name="calledByAct" value="" class="input-border" id="calledByAct"> 
							                    
							                    	<input type="button" value="submit" onclick="postDiscussioValuesAct()" 
							                    	style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;"
							                    	/>
							            </form>
							            
							        </div>
													   			
						  </div>
						
						</div>
		</table>
				<div id="area9" class="areaClass">
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
	<div  class="notification_area" id="notification_area_10">	
	
		<s:form theme="css_xhtml" id="form10">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_10');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label10"></label></td>
				</tr>
		</table>

				<div style="width: 90%;margin-left: 5%;" id="area10" class="areaClass">
				
				</div>	
		
		</s:form>	
	</div>
	<!-- Raju Changes from here-->
	<div  class="notification_area" id="notification_area_20">
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form20">
	<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_20');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label20"></label></td>
				</tr>
		</table>
				<div id="area20" class="areaClass">
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
	<div  class="notification_area" id="notification_area_30"><br><br>
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form30">
	<table class="topClass">	
				<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_30');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label30"></label></td>
				 
				</tr>
		</table>
				<div id="area30" class="areaClass">
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMSToEnquiredMembers" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
	<div  class="notification_area" id="notification_area_31">
		<s:form theme="css_xhtml" id="form31">
	<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_31');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label31"></label></td>
				</tr>
				<div id="myModalForRegMembers" class="modal">
						
						  <!-- Modal content -->
						  <div class="modal-content">
						    <span class="close" onclick="closeModelRegMembers()">&times;</span>
						   
						   			<div >
							            <form  class="form">
							                <legend style="text-align: center;color:white">Follow Up</legend>
							             
							                
							                        <input type="hidden" name="memberIdAct" id="memberIdRegMembers" value="0">
							                    
							                        <label for="member_name" style="color:white">Member Name  </label>:  <input type="text" name="memberNameAct" class="input-border" id="memberNameRegMembers">
							                    
							                    
							                        <label for="reply_date" style="color:white">Replay Date  </label>: <input type="date" name="replyDateAct" class="input-border" id="replyDateRegMembers">
							                    
							                    
							                        <label for="discussion" style="color:white">Discussion  </label>: <textarea name="discussionAct" class="input-border" id="discussionRegMembers" cols="25" rows="3"></textarea>
							                
							                    
							                        <label for="called_by" style="color:white">Called By  </label>: <input type="text" name="calledByAct" value="" class="input-border" id="calledByRegMembers"> 
							                    
							                    	<input type="button" value="submit" onclick="postDiscussioValuesRegMembers()" 
							                    	style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;"
							                    	/>
							            </form>
							            
							        </div>
													   			
						  </div>
						
						</div>
		</table>
				<div id="area31" class="areaClass">
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	<div  class="notification_area" id="notification_area_32"><br><br>
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form32">
	<table class="topClass">	
				<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_32');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label32"></label></td>
				</tr>
				<div id="myModalRegFllPendingFees" class="modal">
						
						  <!-- Modal content -->
						  <div class="modal-content">
						    <span class="close" onclick="closeModelRegFllPendingFees()">&times;</span>
						   
						   			<div >
							            <form  class="form">
							                <legend style="text-align: center;color:white">Follow Up</legend>
							             
							                
							                        <input type="hidden" name="memberIdRegFllPendingFees" id="memberIdRegFllPendingFees" value="0">
							                    
							                        <label for="member_name" style="color:white">Member Name  </label>:  <input type="text" name="memberNameRegFllPendingFees" class="input-border" id="memberNameRegFllPendingFees">
							                    
							                    
							                        <label for="reply_date" style="color:white">Replay Date  </label>: <input type="date" name="replyDateRegFllPendingFees" class="input-border" id="replyDateRegFllPendingFees">
							                    
							                    
							                        <label for="discussion" style="color:white">Discussion  </label>: <textarea name="discussionRegFllPendingFees" class="input-border" id="discussionRegFllPendingFees" cols="25" rows="3"></textarea>
							                
							                    
							                        <label for="called_by" style="color:white">Called By  </label>: <input type="text" name="calledByRegFllPendingFees" value="" class="input-border" id="calledByRegFllPendingFees"> 
							                    
							                    	<input type="button" value="submit" onclick="postDiscussioValuesRegFllPendingFees()" 
							                    	style="width:90px;height:30px;border-radius:10px;background-color:gray;color:white;text-align:center;"
							                    	/>
							            </form>
							            
							        </div>
													   			
						  </div>
						
						</div>
		</table>
				<div id="area32" class="areaClass">
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMSToEnquiredMembers" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
					
				</table>
			
		</s:form>
	</div>
		<!-- Raju Changes to here-->
	
	
	<div  class="notification_area" id="notification_area_11">
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form11">
	<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_11');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label11"></label></td>
				</tr>
		</table>
				<div id="area11" class="areaClass">
				
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	
	<div class="notification_area" id="notification_area_12">
	
		<s:form theme="css_xhtml" id="form12">
			<table class="topClass">
				<tr>
					<td><a href="javascript:void(0);" onclick="closeAction('notification_area_12');"><img alt="" src="images/cancel.png"/></a></td>
					<td><label class="labelClass" id="label12"></label></td>
				</tr>
			</table>
				<div id="area12" class="areaClass" align="center">
					
				</div>
				
				
		</s:form>
		
	</div>
	
	
	<div  class="notification_area" id="notification_area_13">	
	 
		<s:form theme="css_xhtml" id="form13">
		<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_13');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label13"></label></td>
				</tr>
		</table>

				<div style="width: 90%;margin-left: 5%;" id="area13" class="areaClass">
				          
				</div>	
			<table>
					<tr>				
						<td colspan="4"><input type="button" id="saveEsslSetting" class="mybtn" value="Save" class="smallBtn"/>	<td>
					</tr>
				</table>
		</s:form>	
	</div>
	
	
	<div  class="notification_area" id="notification_area_14">
	<button onclick="selectAllEnquiredMembers()" id="selectAllMembersButton">SELECT ALL </button>
		<s:form theme="css_xhtml" id="form11">
	<table class="topClass">	<tr>
				<td><a href="javascript:void(0);" onclick="closeAction('notification_area_14');"><img alt="" src="images/cancel.png"/></a></td>
				<td><label class="labelClass" id="label14"></label></td>
				</tr>
		</table>
				<div id="area14" class="areaClass">
				
				</div>					
				<table>
					<tr>
						<td><label>Send Message :</label></td>
						<td><s:textarea id="txtmessage" name="message" cssStyle="width:300px; height:50px;" value="" /></td>
						<td><s:submit action="sendSMS" value="Send" cssClass="smallBtn"/>
						<td>
					</tr>
				</table>
			
		</s:form>
	</div>
	<!-- </div> -->
	</div> 
	</div>
 </div>	 
 
 	<!-- Edited by raju from here -->
 		<%-- <s:form id="searchMember" theme="css_xhtml"  method="post" enctype="multipart/form-data">
		<!-- 	<div style="width: 90%;"> -->
	<div style="width: 80%;margin-left: 10%;">
			<fieldset>
			<div id="searchDiv" style="height: 60px;cursor:pointer;float:left; width: 20%;">
			<img alt="Search Member" src="<%=request.getContextPath()%>/images/searchMember.jpg" height="60px" width="200px">
			</div>
				
			<div id="searchList" style="height: 60px; width: 60%;float: left;">
				<table>
					<tr>
						
						<td><label>Search By<span class="required">*</span></label></td>
						<td><label>Enter Value<span class="required">*</span></label></td>
						<td></td>
					</tr>
					<tr>
					<td><s:select name="searchBy" id="searchBy"
								cssStyle="width:140px;"
								list="#{'Name':'Name','MobileNo':'MobileNo','MemberId':'MemberId'}" headerKey="" headerValue="---Select---" /></td>
					<td><s:textfield name="searchValue" id="membersNameAutocomplete"></s:textfield></td>
					<td><s:submit align="center" action="searchMemberDetailsByValues" id="search" value="Search" /></td>

					</tr>
				</table>
			</div>
			</fieldset>
			
			<s:token />
		</div> 
	</s:form>
 --%>	
<%--  
 <s:form id="searchMember" theme="css_xhtml"  method="post" enctype="multipart/form-data">
		<!-- 	<div style="width: 90%;"> -->
	<div style="width: 80%;margin-left: 10%;">
			<fieldset>
			<div id="searchDiv" style="height: 50px;cursor:pointer;float:left; width: 20%;">
			<img alt="Search Member" src="<%=request.getContextPath()%>/images/searchMember1.jpg" height="48px" width="140px">
			</div>
				
			<div id="searchList" style="height: 50px; width: 60%;float: left;">
				<table>
					<tr>
						
						<td><label>Member Id</label></td>
						<td><label>Member Name</label></td>
						<td><label>Contact No.<span class="required">*</span></label></td>
					</tr>
					<tr>
					<td><s:hidden name="memberId" id="dispMemberId" value="%{memberId}" /> 
						<s:hidden name="ptStatus" id="ptStatus" value="%{ptStatus}" /> 
							<s:if test="memberId != 0">
								<s:textfield name="displayMemberId" id="displayMemberId1" value="M%{memberId}" placeholder="M1" onkeyup="upperCase(this);"/>
							</s:if>
							<s:else>
								<s:textfield name="displayMemberId" id="displayMemberId1" value="" placeholder="M1" onkeyup="upperCase(this);"/>
							</s:else>
						</td>
						<td><s:textfield tabindex="-1" id="membersNameAutocomplete1" name="membersNameAutocomplete"
								placeholder="First Name Middle Name Last Name" cssStyle="width:250px;" value="%{name}"/></td>
						<td valign="top"><s:textfield tabindex="-1" name="contactNo" id="contactNo1"
								value="%{contactNumber}"/></td>
					   <td><div id="isPT"><b></b></div></td>
					</tr>
				</table>
			</div>
			</fieldset>
			
			<s:token />
		</div> 
	</s:form>
 			
 			
 			<!-- member details form -->
 			<s:form id="MemberDetailsForm" theme="css_xhtml"  method="post" enctype="multipart/form-data">
		<!-- 	<div style="width: 90%;"> -->
	<div style="width: 90%;margin-left: 5%;">
			<fieldset>
				<legend>Personal Details</legend>

				<table>
					<tr>
					<td style="width: 320px;" colspan="1" >Member Name</td>
						<td style="margin-left: 20px;"><label>Member Id<span
								class="required">*</span></label></td>
						<td><label>Contact No.<span class="required">*</span></label></td>
						<td><label>Date of Birth</label></td>
						
					</tr>
					<tr>
					<td colspan="1"><s:textfield id="membersNameAutocomplete" name="membersNameAutocomplete" 
							placeholder="First Name Middle Name Last Name" cssStyle="width:250px;" /></td>
						<td><s:hidden name="memberId" id="memberId" value="" /> 
						<s:textfield
								name="displayMemberId" id="displayMemberId" value=""
								onkeyup="upperCase(this);"  placeholder="M1" /></td>
							<td valign="top"><s:textfield name="contactNo" id="contactNo"
								value="" /></td>
								<td><s:textfield name="dateOfBirth" id="dateOfBirth" value="" />
						
					</tr>
					<tr> 
					<td><label>Joining Date<span class="required">*</span></label></td>
					 </tr>
					<tr><td><s:textfield name="joiningDate" id="joiningDate">
								<s:param name="value">
									<s:date name="new java.util.Date()" format="dd/MM/yyyy" />
								</s:param>
							</s:textfield></td>
					</tr>
				</table>
				<hr />
				<table>
					<tr>
						<td><label>Name of Member<span class="required">*</span></label></td>
						
						<td><s:textfield name="firstName" id="firstName"
								placeholder="First Name" onkeyup="upperCase(this);" value="" /></td>
						<td><s:textfield name="middleName" id="middleName"
								placeholder="Middle Name" onkeyup="upperCase(this);" value="" /></td>
						<td><s:textfield name="lastName" id="lastName"
								placeholder="Last Name" onkeyup="upperCase(this);" value="" /></td>

						<td align="center" rowspan="4" height="120px;">
							<div ng-controller="UploadController" id="updateImage" style="width: 100px; display: none;">
								<!-- <i ng-hide="imageSrc">No image Selected</i> -->
								<div id="imgg" onclick="display()"><i>Take A Picture</i></div>
								<img ng-src="{{imageSrc}}" width="80" height="90" />
								<s:file ng-file-select="onFileSelect($files)" id="userImage"
									name="userImage" cssStyle="width: 70px; margin:0 auto;" onclick="hide()"/> 
							</div>
							
							<div id="image" class="gallery-item" style="width: 100px;" onclick="display();"></div>
														
							
						</td>
						<td align="center" rowspan="4" height="120px;">
						<video id="videoID" autoplay style="width: 90px; height: 90px; display: none;"></video>
							<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="button" value="Take photo" id="captureID" onclick="capture()" style="width: 70px; height: 30px; display: none;" />
							
						</td>
						<td align="center" rowspan="4" height="120px;">
							
							<canvas id="canvasID" width="90" height="90" style="display: none;"></canvas>
							
							<br>&nbsp&nbsp&nbsp&nbsp<input id="save" type="button" value="Save" style="width: 70px; height: 30px; display: none;" onclick="send(); this.disabled=true;return true;" />
						</td>
					</tr>
					<tr>
						<td><label>Gender<span class="required">*</span></label></td>
						 
							<td><label>Age<span class="required">*</span></label></td>
							 <td><label>Marital Status<span class="required">*</span></label></td>
					<td class="annDate" style="display:none;"><label>Anniversary Date</label></td>
					
					</tr>
					<tr>
						<td><s:select name="gender" id="gender"
								cssStyle="width:140px;"
								list="#{'Male':'Male','Female':'Female'}"  headerKey="" headerValue="Select Gender"/></td>
						
						<td><s:textfield name="age" id="age" value="" /></td>
						<td><s:select name="maritalStatus" id="maritalStatus" cssStyle="width:140px;"
								list="#{'Unmarried':'Single','Married':'Married'}" headerKey="" headerValue="Select Marital Status"/>  </td>
								
						<td class="annDate" style="display:none;"><s:textfield name="anniversaryDate" id="anniversaryDate">
								<s:param name="value">
									<s:date name="new java.util.Date()" format="dd/MM/yyyy" />
								</s:param>
							</s:textfield>
					</td>
					<td></td>
						<td></td>
					<td></td>
					</tr>
					<tr height="40px;"></tr>
				</table>
				<hr />
				<table>
					<tr>
						<td><label>Personal Trainer</label></td>
						<td><s:select id="personalTrainerId" name="ptNameList" headerKey="0" 
							headerValue="Select PT" cssStyle="width:140px;" list="ptNameMap" value="" /></td>
					</tr>

				</table>
			</fieldset>
			<fieldset>
				<legend>Contact Details</legend>
				<table>
					<tr>
						<td><label>Residential Address</label></td>
						
						<td><label>Alternate Contact No.</label></td>
						<td><label>E-Mail Id</label></td>
					</tr>
					<tr>
						<td><s:textarea name="address" id="address"
								onkeyup="firstCharWordUpper(this);" value="" /></td>
						
						<td valign="top"><s:textfield name="alternateContactNo"
								id="alternateContactNo" value="" /></td>
						<td colspan="2"><s:textfield name="emailId" id="emailId"
								value="" cssStyle="width:180px;"></s:textfield></td>
					</tr>
				</table>
			</fieldset>
			<fieldset>
				<legend>Physical Details</legend>
				<table style="margin: 0 auto;">
					<tr>
						<td><label>Height </label></td>
						<td><label>Weight</label></td>
						<td><label>Chest</label></td>
						<td><label>Bicep</label></td>
						<td><label>Tricep</label></td>
					</tr>
					<tr>
						<td><s:textfield name="height" id="height"
								cssStyle="width:90px;" value="" /></td>
						<td><s:textfield name="weight" id="weight"
								cssStyle="width:90px;" value="" /></td>
						<td><s:textfield name="chest" id="chest"
								cssStyle="width:90px;" value="" /></td>
						<td><s:textfield name="bicep" id="bicep"
								cssStyle="width:90px;" value="" /></td>
						<td><s:textfield name="tricep" id="tricep"
								cssStyle="width:90px;" value="" /></td>
					</tr>
				</table>
			</fieldset>
		<s:token />
		</div>
	</s:form>
 --%> 			<!-- member details form end here -->
			<!-- Edited by raju to here -->
</body>

 
<script type="text/javascript">
	
	/* 	var today = new Date();		
		var dd = today.getDate();
		var mm = today.getMonth()+1; //January is 0!
		var yyyy = today.getFullYear();
		if(dd<10){
   			 dd='0'+dd
		} 
		if(mm<10){
   			 mm='0'+mm
		} 
		var today = yyyy+"-"+mm+"-"+dd;
		var oneday=1000*60*60*24;	 */	

/* 	$("#document").ready(function(){	 */	
	$(document).on('ready readyAgain', function() {			
	
		//====================== For BirthDay ==================================    
		    $('#hiddenFromDate').datepicker({
		    	changeMonth: true,
				changeYear: true,
				dateFormat: 'dd/mm/yy'				
		      ,onSelect: function(dateText, inst) {
		          $('#fromBD').text(dateText);$('#toBD').text(dateText);
		      },onClose : function(date){
					$("#hiddenToDate").datepicker( "option", "minDate", date );	
					 loadBDList(true);
				}});
		  	
		  	
		  	$('#hiddenToDate').datepicker({
		    	changeMonth: true,
				changeYear: true,
				dateFormat: 'dd/mm/yy',
				minDate:  0
		      ,onSelect: function(dateText, inst) {
		          $('#toBD').text(dateText);
		      },onClose : function(date){
					///;	
		    	  loadBDList(true);
				}});
		  	
		  	$("#hiddenToDate").prop( "option", $('#hiddenFromDate').val() );	
		  	$('#fromBD').click(function(){
		        $('#hiddenFromDate').datepicker('show');		        
		     });
		  	
		  	$('#toBD').click(function(){
		        $('#hiddenToDate').datepicker('show');		        
		      });	
		  //====================== For BirthDay ==================================    
		//====================== For Anniversary ==================================    
		    $('#hiddenFromAnDate').datepicker({
		    	changeMonth: true,
				changeYear: true,
				dateFormat: 'dd/mm/yy'				
		      ,onSelect: function(dateText, inst) {
		          $('#fromAnD').text(dateText);$('#toAnD').text(dateText);
		      },onClose : function(date){
					$("#hiddenToAnDate").datepicker( "option", "minDate", date );	
					 loadAnList(true);
				}});
		  	
		  	
		  	$('#hiddenToAnDate').datepicker({
		    	changeMonth: true,
				changeYear: true,
				dateFormat: 'dd/mm/yy',
				minDate:  0
		      ,onSelect: function(dateText, inst) {
		          $('#toAnD').text(dateText);
		      },onClose : function(date){
					///;	
		    	  loadAnList(true);
				}});
		  	
		  	$("#hiddenToAnDate").prop( "option", $('#hiddenFromAnDate').val() );	
		  	$('#fromAnD').click(function(){
		        $('#hiddenFromAnDate').datepicker('show');		        
		     });
		  	
		  	$('#toAnD').click(function(){
		        $('#hiddenToAnDate').datepicker('show');		        
		      });	
		  //====================== For Anniversary ================================== 	  
			//====================== For Renew Package By date ==================================    
		    $('#hiddenFromReNewDate').datepicker({
		    	changeMonth: true,
				changeYear: true,
				dateFormat: 'dd/mm/yy'				
		      ,onSelect: function(dateText, inst) {
		          $('#fromReNewD').text(dateText);$('#toReNewD').text(dateText);
		      },onClose : function(date){
					$("#hiddenToReNewDate").datepicker( "option", "minDate", date );	
					loadReNewPackageList(true);
				}});
		  	
		  	
		  	$('#hiddenToReNewDate').datepicker({
		    	changeMonth: true,
				changeYear: true,
				dateFormat: 'dd/mm/yy',
				minDate:  0
		      ,onSelect: function(dateText, inst) {
		          $('#toReNewD').text(dateText);
		      },onClose : function(date){
					///;	
		    	  loadReNewPackageList(true);
				}});
		  	
		  	$("#hiddenToReNewDate").prop( "option", $('#hiddenFromReNewDate').val() );	
		  	$('#fromReNewD').click(function(){
		        $('#hiddenFromReNewDate').datepicker('show');		        
		     });
		  	
		  	$('#toReNewD').click(function(){
		        $('#hiddenToReNewDate').datepicker('show');		        
		      });	
		  //======================  For Renew Package ==================================    
			  
			  
		//Auto Complete for Search By Member Name		
		//autoComplete("membersNameAutocompleteNotification","displayNotificationMemberId",onMemberIdChange1);		
		 //Auto Complete for Search By number
/* 		autoCompleteSearchMember("contactNoNotification","displayNotificationMemberId",onMemberIdChange1);	
 */		 
		$("#membersNameAutocompleteNotification").keyup(function(){			
			//alert('inside membersNameAutocompleteNotification......');
			homepageAutoComplete("membersNameAutocompleteNotification","displayNotificationMemberId",onMemberIdChange1);
		});
		
		// fetch member details
		$("#displayNotificationMemberId").change(function(){
			//alert('inside displayNotificationMemberId');
			onMemberIdChange1();						//Fetch Members Details On Change of Member Id
		});
		
		//Save In time
		$("#in").click(function(){
			var now = new Date($.now());
			
			var time24HR = ((now.getHours() >= 10) ? now.getHours() : "0"+now.getHours())
			+":"+((now.getMinutes() >= 10) ? now.getMinutes() : "0"+now.getMinutes());
			
			//if(now.getHours()>10) 
			var time12HR = getTimeIn12Hour(time24HR);
			
			$("#inTime").val(time12HR);
			$("#inTime").show();
			$("#in").hide();
			var memberId = $("#memberId").val();
			$.getJSON('saveInTime',{inTime : time24HR , memberId : memberId}).done(function(data){
				$("#attendanceId").val(data.attendanceId);
				$("#txtmessage").text("").hide();
			});
		});
		
		//Save Out Time
		$("#out").click(function(){
			
		
			if($("#inTime").val()!="00:00"){
				$("#txtmessage").text("").hide();
				var now = new Date($.now());
			
				//var outTime = now.getHours()+":"+now.getMinutes();
				var time24HR = ((now.getHours() >= 10) ? now.getHours() : "0"+now.getHours())
				+":"+((now.getMinutes() >= 10) ? now.getMinutes() : "0"+now.getMinutes());
				
				var time12HR = getTimeIn12Hour(time24HR);				
				$("#outTime").val(time12HR);
				$("#outTime").show();
				$("#out").hide();
				var attendanceId = $("#attendanceId").val();
				$.getJSON('saveOutTime',{outTime : time24HR , attendanceId : attendanceId}).done(function(data){});
			}
			else{
				$("#message").text("Please click on In Time First...").show();
			}
		});
		
		//fingerprint Detail/////
		$("#testFing").click(function(){
			$("#scanMessage").hide();
				var memberId = $("#memberId").val();
				//alert(memberId);
								
				$.getJSON('matchFingerPrint',{memberId : memberId}).done(function(data){});
				
				$("#scanMessage").text("Member Verified").show();
				//alert(data[0].currentId);
				/* 	$.each(data.records, function(i, f) {
	    		    var id =  f.currentId ;
	    		    alert(id);
	           		}); */
					
				//alert("2: " +memberId);
		});////// end of finger print Detail
		
		$("#displayNotificationMemberId").focus();
		
	}); //End Of Document ready   
	
	
	
	
	/* var onMemberIdChange1= function(){
		
		var memberId = $("#displayNotificationMemberId").val().substring(1);
		var checkMemberId = $("#displayNotificationMemberId").val().substring(0,1);
		
		if(memberId > 0 && checkMemberId =='M'){
			
			$("#memberId").val(parseInt(memberId));
			$.getJSON('fetchPendingFeesDetails'{memberId : memberId}).done(function(data){
				if(data.)
			})
			
		}
	} */
	
	

	//Function to Fetch Members Details On Change of Member Id
	var onMemberIdChange1 = function(){
		//alert('inside onMemberIdChange1 ');
		$("#errorContainer").empty();
		$('<div class="errors" id="message" style="width: 530px;" style="display:none;"></div>').appendTo($("#errorContainer"));	
		NotificationClearForm();	
		var memberId=$("#displayNotificationMemberId").val();//.substring(1);
		//var checkMemberString=$("#displayNotificationMemberId").val().substring(0,1);		
		if($("#displayNotificationMemberId").val().substring(0,1)=='M'){
			memberId=$("#displayNotificationMemberId").val().substring(1);
			$("#displayNotificationMemberId").val(memberId);
		}
		
		
		if(memberId > 0 )//&& checkMemberString == 'M')
		{
			
			$("#memberId").val(parseInt(memberId));		
			$.getJSON('fetchMemberAttendance',{memberId : memberId}).done(function(data){		//Json To Fetch Data	
				
				if(data.memberStatusList[0] != undefined){						
					
						if(data.memberStatusList[0].status=="registered" && data.memberDetailsList[0] != undefined){
							
							assignNotificationData(data.memberDetailsList[0],data.attendanceDetailsList[0],data.pendingFeesDetailsList[0]);
							//assignNotificationDataToTable(data.pendingFeesDetailsList[0]);						
							/* if(today > data.memberDetailsList[0].endDate) */
							 	if(( (  new Date(data.memberDetailsList[0].endDate).getTime() - new Date(today).getTime())/(oneday)) < 0 )
							 	{
							 		//data.pendingFeesDetailsList[0]
							 								 		
							 		//alert('end date: '+data.memberDetailsList[0].endDate);
							 		var t1 = new Date(data.memberDetailsList[0].endDate).getTime();
							 		var t2 = new Date(today).getTime();
							 		var t3 =  new Date(data.memberDetailsList[0].endDate).getTime() - new Date(today).getTime();
							 		var t4 =  (  new Date(data.memberDetailsList[0].endDate).getTime() - new Date(today).getTime())/(oneday);
							 		
									$("#tr").css("background","linear-gradient(rgb(250, 200, 200), rgb(250, 100, 100))");  //red
								}
							
							 
								 else
								 {
								 	if(( (  new Date(data.memberDetailsList[0].endDate).getTime() - new Date(today).getTime())/(oneday)) > 5 )
									/* if(today < data.memberDetailsList[0].endDate) */	{
										$("#tr").css("background", "linear-gradient(rgb(170, 250, 170), rgb(50, 250, 50))"); //green
								 	}
							 		else 
									{ 
										$("#tr").css("background","linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");  //yellow
									 }
								
								 }			
								
							/* else if(( (new Date(data.memberDetailsList[0].endDate).getTime() - new Date(today).getTime())/(oneday)) <= 5 && ( (new Date(list[0].endDate).getTime() - new Date(today).getTime())/(oneday)) >= 0)
										 $("#tr").css("background","linear-gradient(rgb(250, 250, 200), rgb(250, 250, 50)");  //yellow */
						}else{									
							
								assignNotificationStatusData(data.memberStatusList[0]);
								$("#message").text("Member has not paid fees.").show();
							 }	
						
				
				}else{
						$("#message").text("Member Id is not Registered. Please Enter Valid Member Id...").show();
					 }
			})
			.fail(function(jqxhr, textStatus, error) {
				$("#message").text("Error While Fetching. Please Try Again...").show();
			});
		}
		else
			$("#message").text("Member Id is not valid. Please Enter Valid Member Id...(ex. 1,2)").show();
	};
	
	//Assign Data

	/* var assignNotificationData = function(memberData,attendanceData,pendingFeesDetailsData){	
		
		//alert(pendingFeesDetailsData.ledgerName);
		$("#attendanceDetails").show();
		$("#fieldset2").show();
		//alert("attendanceDetails");
 */
	 var assignNotificationStatusData = function(memberData){
		 var ctx ='<%=request.getContextPath()%>';	
		 $('<img src="'+ctx+'/MemberImages/'+memberData.memberId+'.jpg"  width="80px" hieght="90px" id="imageMember" alt="Image Not Added" />').appendTo($("#image"));
			
			$("#membersNameAutocompleteNotification").val(memberData.firstName+" "+memberData.middleName+" "+memberData.lastName); 
			$("#memberName").text(memberData.firstName+" "+memberData.middleName+" "+memberData.lastName);
			$("#contactNoNotification").val(memberData.contactNo);
	 };
	 
	
	 
	var assignNotificationData = function(memberData,attendanceData,pendingFeesDetailsData){
		 var ctx ='<%=request.getContextPath()%>';	
		$("#attendanceDetails").show();
		$("#fieldset2").show();	
		
		 $('<img src="'+ctx+'/MemberImages/'+memberData.memberId+'.jpg"  width="80px" hieght="90px" id="imageMember" alt="Image Not Added" />').appendTo($("#image"));
		
		$("#membersNameAutocompleteNotification").val(memberData.firstName+" "+memberData.middleName+" "+memberData.lastName); 
		$("#memberName").text(memberData.firstName+" "+memberData.middleName+" "+memberData.lastName);
		$("#contactNoNotification").val(memberData.contactNo);

		//$("#feesName").text(pendingFeesDetailsData.ledgerName);
		$("#joiningDate").text(convertToUserDate(memberData.joiningDate));
		$("#startDate").text(convertToUserDate(memberData.startDate));
		$("#endDate").text(convertToUserDate(pendingFeesDetailsData.endDate));
		$("#dueDate").text(convertToUserDate(pendingFeesDetailsData.dueDate));
		$("#totalFees").text(pendingFeesDetailsData.ledgerTotalFees);
		$("#paidFees").text(pendingFeesDetailsData.ledgerPaidFees);
		$("#pendingFees").text(pendingFeesDetailsData.ledgerPendingFees);
		
		//$("#startDate").text(convertToUserDate(pendingFeesDetailsData.startDate));
		//$("#endDate").text(convertToUserDate(pendingFeesDetailsData.endDate));
	
		

	    if(memberData.status=="registered"){  
		         $("#attendanceDetails").show();
		if(attendanceData != undefined)
		{
						
			if(attendanceData.inTime != null){
				$("#inTime").show();
				$("#in").hide();
				var time24HR = attendanceData.inTime;
				var time12HR = getTimeIn12Hour(time24HR);
				
				$("#inTime").val(time12HR);
			}
			if(attendanceData.outTime != null){
				$("#outTime").show(); 
				$("#out").hide();
				
				var time24HR = attendanceData.outTime;
				var time12HR = getTimeIn12Hour(time24HR);
				$("#outTime").val(time12HR);
			}
			$("#attendanceId").val(attendanceData.attendanceId);
		}				
	    }else{
	    	$("#message").text("Member Id is not currently Registered. He is removed. ").show();
	    }
	};	
		
	var getTimeIn12Hour=function(time){		
		var hh=parseInt(time.split(":")[0]);
		var mm=parseInt(time.split(":")[1]);
		var time12HR;var AMorPM;
		if(hh>=12){
			if(hh>12)		
				time12HR=hh-12;
			else
				time12HR=hh;
			
			time12HR=time12HR+":"+mm;
			AMorPM=" PM";
		}
		else if(hh<12){
			time12HR=time;
			AMorPM=" AM";
		}
		 hh=parseInt(time12HR.split(":")[0]);
		 mm=parseInt(time12HR.split(":")[1]);
		time12HR=((hh >= 10) ? time12HR.split(":")[0] : "0"+hh)	+":"+((mm >= 10) ? mm : "0"+mm)+""+AMorPM;		
		return time12HR;
	};
	//Clear all Data
	var NotificationClearForm = function(){	
		$("#message").hide();
		$("#attendanceDetails").hide();
		$("#fieldset2").hide();
		$("#imageMember").remove();
		$("#image").val("");
		$("#memberName").text("");
		$("#attendanceId").val(0);
		$("#inTime").hide();
		$("#in").show();
		$("#inTime").val("00:00");
		$("#outTime").hide();
		$("#out").show();
		$("#outTime").val("00:00");
		$("#membersNameAutocompleteNotification").val("");
		$("#contactNoNotification").val("");		
		
	};
	
	//Personal Trainer Ready Function
	$(document).on('ready readyAgain1', function(){	
		$("#errorContainer").empty();
		
		//action="saveEsslSettingAction"
		$("#saveEsslSetting").click(function(){					
			$('<div class="errors" id="message" style="width: 530px;display:none;"></div>').appendTo($("#errorContainer"));				
			var esslIP=$("#esslIP").val();
			$("#saveEsslSetting").prop("disabled",true);
			var serverName=$("#serverName").val();
			var portNo=$("#portNo").val();
			var databaseName=$("#databaseName").val();			
			   $.ajax({url: "saveEsslSettingAction",  data:{serverName : serverName , portNo : portNo,databaseName:databaseName,esslIP:esslIP},type: 'GET',/* async: false, */
				   success: function(result,status,xhr){			 
					   //alert("success="+status);
		            },beforeSend:function(xhr){
		            	//$("#wait").css("display", "block");
		            },complete:function(xhr,status){
		            	//$("#wait").css("display", "none");
		            	if(status=="success"){
		            		$("#errorContainer").empty();		            	
		            		$('<div class="welcome" id="message" style="width: 530px;display:none;"></div>').appendTo($("#errorContainer"));
		            		$("#message").text("Detail Saved").show();
		            	}		            		
		            	else{
		            		$("#message").text("Error While Saving. Please Try Again...").show();
		            	}		            		
		            	$("#saveEsslSetting").prop("disabled",false);
		            }
			   
			   }).fail(function(jqxhr, textStatus, error) {
					$("#message").text("Error While Saving. Please Try Again...").show();
				});
			   
			    
			
			/* $.getJSON('saveEsslSettingAction',{serverName : serverName , portNo : portNo,databaseName:databaseName,esslIP:esslIP}).done(function(data){
				alert("Details Saved..");
				//$(selector).getJSON(url,data,success(data,status,xhr))
				
			})
			.fail(function(jqxhr, textStatus, error) {
				$("#message").text("Error While Saving. Please Try Again...").show();
			});			
			 	$(document).ajaxStart(function(){
			        $("#wait").css("display", "block");
			    });
			    $(document).ajaxComplete(function(){
			        $("#wait").css("display", "none");
			    }); 			    
			 */
			
		});
		//Autocompleter For Personal Trainer By Contact No
		autoCompletePTNumber("contactNoNotification","displayNotificationPersonalTrainerId",onPersonalTrainerIdChange); 	
		
		/* Edited By Raju Sable */
		//Auto Complete for Search By Member Name
	//autoComplete("membersNameAutocomplete1","displayMemberId1",onMemberIdChange);
	$("#membersNameAutocomplete1").keyup(function(){			
		homepageAutoComplete("membersNameAutocomplete1","displayMemberId1",onMemberIdChange);
	});
	
	// fetch member details
	 $("#displayMemberId1").change(function(){
		onMemberIdChange();						//Fetch Members Details On Change of Member Id
	}); 
	
	//Auto Complete for Search By number
		$("#contactNo1").keyup(function(){			
			autoCompleteSearchMemberByContact("contactNo1","displayMemberId1",onMemberIdChange);
		});
	
	//On Member Id Change Event get member details, Fees details
	$("#displayMemberId1").blur(function(){
			onMemberIdChange();							//Fetch Members Details On Change of Member Id
		});

	var onMemberIdChange = function(){
		
	
		$('<div class="errors" id="message" style="width: 530px;display:none;"></div>').appendTo($("#errorContainer"));
		var fetchMemberId = $("#displayMemberId1").val().substring(1);
		var checkMemberString=$("#displayMemberId1").val().substring(0,1);
		clearForm();
		if(fetchMemberId > 0 && checkMemberString == 'M')
		{
			$("#memberId").val(parseInt(fetchMemberId));
			$.getJSON('fetchMemberDetails',{fetchMemberId : fetchMemberId}).done(function(data){
				
				if(data.memberList[0] == undefined)
					$("#message").text("Member Id is not Registered. Please Enter Valid Member Id...").show();
				else
					assignData(data.memberList[0]);
			})
			.fail(function(jqxhr, textStatus, error) {
				$("#message").text("Error While Fetching. Please Try Again...").show();
			});
		}
		else
			$("#message").text("Member Id is not valid. Please Enter Valid Member Id...(ex. M1,M2)").show();
	};	
	
	//Clear Form
	var clearForm = function() {
		$("#imageMember").remove();
		//$("#message").show();
		$("#update").attr("disabled", true);
		$("#joiningDate").val("");
		$("#lastName").val("");
		$("#firstName").val("");
		$("#middleName").val("");
		$("#gender").val("");
		$("#dateOfBirth").val("");
		$("#address").val("");
		$("#contactNo").val("");
		$("#alternateContactNo").val("");
		$("#emailId").val("");
		$("#height").val("");
		$("#weight").val("");
		$("#chest").val("");
		$("#bicep").val("");
		$("#tricep").val("");
		$("#imageMember").remove();
		$("#memberId").val(0);
		$("#age");
		$("#maritalStatus").val("");
		$("#membersNameAutocomplete").val("");
		$("#anniversaryDate").val("");
	};
	
	//Assign Data
	var assignData = function(data){
		
		$("#MemberDetailsForm").show();

		$("#message").hide();
		$("#update").attr("disabled",false);
		$("#joiningDate").val(convertToUserDate(data.joiningDate)); 
		$("#lastName").val(data.lastName);
		$("#firstName").val(data.firstName);
		$("#middleName").val(data.middleName);
		$("#gender").val(data.gender);
		$("#dateOfBirth").val(convertToUserDate(data.dateOfBirth));
		$("#address").val(data.address);
		$("#age").val(data.age);
		$("#contactNo").val(data.contactNo);
		$("#dispMemberId").val(data.memberId);
		$("#displayMemberId").val("M"+data.memberId);
		$("#displayMemberId1").val("M"+data.memberId);
		$("#contactNo1").val(data.contactNo);
		$("#memberId").val(data.memberId);
		$("#alternateContactNo").val(data.alternateContactNo);
		$("#emailId").val(data.emailId);
		$("#height").val(data.height);
		$("#weight").val(data.weight);
		$("#chest").val(data.chest);
		$("#bicep").val(data.bicep);	
		$("#maritalStatus").val(data.maritalStatus);
		$('#anniversaryDate').val("");			
		if(data.maritalStatus=="Married"){	
			 $('.annDate').show();
			$("#anniversaryDate").val(convertToUserDate(data.anniversaryDate));		
			$('#anniversaryDate').attr("disabled",false);
		} 	else{
			$("#maritalStatus").val("Unmarried");
			$('#anniversaryDate').attr("disabled",true);
			 $('.annDate').hide();
		}	
		$("#personalTrainerId").val(data.ptNameList);
		$("#tricep").val(data.tricep);
		$("#membersNameAutocomplete").val(data.firstName+" "+data.middleName+" "+data.lastName);
		$("#image").append('<img src="<%=request.getContextPath()%>/MemberImages/'+data.memberId+'.jpg"  width="80px" hieght="90px" id="imageMember" title="Change Image" alt="Image Not Added" />');
	};
	
	
	/* Edited by raju to here */
		//Autocompleter Foe Personal Trainer By Names
		$("#personalTrainerNameAutocompleteNotification").keyup(function(){			
			autoCompletePT("personalTrainerNameAutocompleteNotification","displayNotificationPersonalTrainerId",onPersonalTrainerIdChange);
		});
		
		$("#displayNotificationPersonalTrainerId").change(function(){
			
			onPersonalTrainerIdChange();
		});	
		
		//Save Personal Trainer In Time
		$("#in").click(function(){
			var now = new Date($.now());
			
			//var inTime = now.getHours()+":"+now.getMinutes();
			var time24HR = ((now.getHours() >= 10) ? now.getHours() : "0"+now.getHours())
			+":"+((now.getMinutes() >= 10) ? now.getMinutes() : "0"+now.getMinutes());			
			var time12HR = getTimeIn12Hour(time24HR);
			$("#inTime").val(time12HR);
			$("#inTime").show();
			$("#in").hide();
			var personalTrainerId = $("#personalTrainerId").val();
			$.getJSON('savePTInTime', {inTime : time24HR, personalTrainerId : personalTrainerId}).done(function(data){
				$("#attendanceId").val(data.attendanceId);
				$("#txtmessage").text("").hide();
			});
			
		});
		
		//Save Personal Trainer Out Time
		$("#out").click(function(){
			
		
			if($("#inTime").val() != "00:00"){
				
			
				$("#txtmessage").text("").hide();
				var now = new Date($.now());
				//var outTime = now.getHours()+":"+now.getMinutes();
				var time24HR = ((now.getHours() >= 10) ? now.getHours() : "0"+now.getHours())
				+":"+((now.getMinutes() >= 10) ? now.getMinutes() : "0"+now.getMinutes());				
				var time12HR = getTimeIn12Hour(time24HR);
				$("#outTime").val(time12HR);
				$("#outTime").show();
				$("#out").hide();
				var attendanceId = $("#attendanceId").val();
				$.getJSON('savePTOutTime',{outTime : time24HR , attendanceId : attendanceId}).done(function(data){});
			}
			else{
				$("#message").text("Please click on In Time First...").show();
			}
		});
		
		$("#displayNotificationPersonalTrainerId").focus();
		
	});//End of Document Ready
	
	//Function to Fetch Personal Trainer Details On Change of Member Id
	var onPersonalTrainerIdChange = function(){
		//alert('inside onPersonalTrainerIdChange');
		$("#errorContainer").empty();
		$('<div class="errors" id="message" style="width:530px;" style="display: none;"></div>').appendTo($("#errorContainer"));
		PTNotificationClearForm();
		var personalTrainerId =$("#displayNotificationPersonalTrainerId").val();
		
		if($("#displayNotificationPersonalTrainerId").val().substring(0,2)=='PT'){
			personalTrainerId=$("#displayNotificationPersonalTrainerId").val().substring(2);
			$("#displayNotificationPersonalTrainerId").val(personalTrainerId);
			
		}
		
		if(personalTrainerId > 0)
		{
			
		
			$("#personalTrainerId").val(parseInt(personalTrainerId));
			$.getJSON('fetchPersonalTrainerAttendance',{personalTrainerId : personalTrainerId}).done(function(data){
				
				if(data.personalTrainerStatusList[0] != undefined){
					
					if(data.personalTrainerStatusList[0].statusPT == "joined" && data.personalTrainerDetailsList[0] != undefined)
						assignPTNotificationData(data.personalTrainerDetailsList[0], data.attendanceDetailsPTList[0]);
					}else{
						$("#message").text("Personal Trainer Id is not Registered. Please Enter Valid Personal Trainer Id...").show();
					}
				
				 })
				.fail(function(jqxhr, textStatus, error) {
					$("#message").text("Error While Fetching. Please Try Again...").show(); 
				});
			}
			else
				$("#message").text("Personal Trainer Id is not valid. Please Enter Valid Personal Trainer Id...(ex. 1,2)").show();
		};
		
		var assignPTNotificationData = function(personalTrainerData, attendancePTData){
			
			<%-- var ctx = '<%=request.getContextPath()%>'; --%>
			$("#attendanceDetails").show();
			
			//$('<img src"'+ctx+'//'+personalTrainerData.personalTrainerId+'.jpg" width="80px" height="90px" id="imageMember" alt="Image Not Added" />').appendTo($("#image"));
			
			$("#personalTrainerNameAutocompleteNotification").val(personalTrainerData.firstNamePT+" "+personalTrainerData.middleNamePT+" "+personalTrainerData.lastNamePT);
			$("#personalTrainerName").text(personalTrainerData.firstNamePT+" "+personalTrainerData.middleNamePT+" "+personalTrainerData.lastNamePT);
			$("#contactNoNotification").val(personalTrainerData.contactNoPT);
			
			if(personalTrainerData.statusPT == "joined"){
				
				$("#attendanceDetails").show();
				if(attendancePTData != undefined)
				{
					if(attendancePTData.inTime != null){
						$("#inTime").show();
						$("#in").hide();
						
						var time24HR = attendancePTData.inTime;
						var time12HR = getTimeIn12Hour(time24HR);
						$("#inTime").val(time12HR);
					}
					if(attendancePTData.outTime != null){
						$("#outTime").show();
						$("#out").hide();
						var time24HR = attendancePTData.outTime;
						var time12HR = getTimeIn12Hour(time24HR);
						$("#outTime").val(time12HR);
					}
					$("#attendanceId").val(attendancePTData.attendanceId);
				}
				
			}else{
		    	$("#message").text("Personal Trainer Id is not currently Registered. He is removed. ").show();
		    }
		};
		
		//Clear Personal Trainer all Data
		var PTNotificationClearForm = function(){	
			$("#message").hide();			
			$("#attendanceDetails").hide();
			//$("#fieldset2").hide();
			//$("#imageMember").remove();
			//$("#image").val("");
			$("#personalTrainerName").text("");
			$("#attendanceId").val(0);
			$("#inTime").hide();
			$("#in").show();
			$("#inTime").val("00:00");
			$("#outTime").hide();
			$("#out").show();
			$("#outTime").val("00:00");
			$("#personalTrainerNameAutocompleteNotification").val("");
			$("#contactNoNotification").val("");		
			
		}; 
</script>
<s:if test="%{#session.roleCount==0}">
<script type="text/javascript">
$('#notificationField').attr('disabled',true);
$('#notificationField').empty();
$('#notificationField').attr("onclick","showErrorMessage();return false");
var showErrorMessage=function(){	
	alertMsg("You are not authenticate person.Please contact admin.");	
};
</script>
</s:if>
<div id="wait" style="display:none;border:0px solid black;position:absolute;top:50%;left:50%;padding:2px;"> 
<img src='<%=request.getContextPath()%>/images/wait2.gif' width="100%" height="100%" alt="Request Processing...." style="font-family: Georgia; color: #697c52; font-style: italic; font-size: 30px;text-align:center;"/><br></div>
<div id="audio"></div>
<script type="text/javascript">	 
		var width = screen.availWidth;
		var height = screen.availHeight;		
		$("#wait").width(width/2).height(height/2);	
		$("#wait").css({ top: width/16+'px',left:height/2+'px' });
		//width:70px;height:70px;			
		function beep1() {
		    // Beep Sound For Presenty
			var snd = new Audio("data:audio/wav;base64,");  
		    snd.play();	
		    
		}
		
		function beep11() {
			 // Beep Sound For Member having Pending Fees
			var snd = new Audio("data:audio/wav;base64,");
			 snd.play();
			 
		}
		function beep2() {
			 // Beep Sound For Member having Pending Fees
			var snd = new Audio("data:audio/wav;base64,");  
		    snd.play();
		    
		}
</script>

<script>
var checkBoxFlag=false;
function selectAllEnquiredMembers()
{
	
	console.log("hello from here");
	//$.ajax({url:'sendMessageToMemberSupposedToGiveFees',success:function(data){},complete:function(){xhr,status}  });
	
	

	var chechboxes=document.querySelectorAll('.member > table > tbody > tr > td > input');
	
	if(checkBoxFlag==false)
	{
		
		
		for(let checkbox of chechboxes)
			{
				checkbox.checked=true;
				
			}
		
		document.getElementById("selectAllMembersButton").innerText="UNSELECT ALL";
		checkBoxFlag=true;	
		 
	}
	else
	{
		document.getElementById("selectAllMembersButton").innerText="SELECT ALL";
		for(let checkbox of chechboxes)
		{
			checkbox.checked=false;
			
		}
		checkBoxFlag=false;
		 
	}
}


</script>

<script>
// Get the modal
	var modal;
 function openModel(memberId,memberName) {
	
	 //myModalForActiveUsers
	 modal = document.getElementById("myModal");
	 modal.style.display = "block";
	 $("#memberId").val(memberId);
	 $("#memberName").val(memberName);
	 console.log(modal);
	 $.getJSON("getPendingFeesDiscusionDetails",{memberId:memberId},function(res,status,xhr){
			
			 console.log(res);
			if(res.pendingFeesDiscusionDetails[0].memberId=="")
			{
				
				console.log("In opemmmm model",res);
				$("#memberId").val(memberId);
				$("#memberName").val(memberName);	
				$("#replyDate").val("");
				$("#discussion").val("");
				let per = $("#loggedUser").val();
				$("#calledBy").val(per);	
				return;
				
			}
			else
			{
				console.log("In opemmmm ",res);
				$("#memberId").val(res.pendingFeesDiscusionDetails[0].memberId);
				$("#memberName").val(res.pendingFeesDiscusionDetails[0].memberName);
				$("#replyDate").val(res.pendingFeesDiscusionDetails[0].replyDate);
				$("#discussion").val(res.pendingFeesDiscusionDetails[0].discussion);
				$("#calledBy").val(res.pendingFeesDiscusionDetails[0].calledBy);	
			}
			
		});
	
}

// When the user clicks on <span> (x), close the modal
 function closeModel() {
	 document.getElementById("myModal").style.display = "none";
	 $("#memberId").val("");
	 $("#memberName").val("");
	 $("#replyDate").val("");
	 $("#discussion").val("");
	 $("#calledBy").val("");
}


// When the user clicks anywhere outside of the modal, close it
window.onclick = function(event) {
  if (event.target == modal) {
    modal.style.display = "none";
    $("#memberId").val("");
	 $("#memberName").val("");
	 $("#replyDate").val("");
	 $("#discussion").val("");
	 $("#calledBy").val("");
  }
}
					function postDiscussioValues()
					{
						 data={
							 memberId :$("#memberId").val(),
							 memberName :$("#memberName").val(),
							 replyDate :$("#replyDate").val(),
							 discussion :$("#discussion").val(),
							 calledBy :$("#calledBy").val()
							 
						}
						console.log(data);
						$.post("pendingFeesDiscusionDetails",data,function(res){
					
							console.log(data);
						});
						location.reload();
						closeModel();
					}
</script>

<script>
	//RegFllPendingFees--Model for Regular follow up for pending fees
		var modalRegFllPendingFees;
 function openModelRegFllPendingFees(memberId,memberName) {
	
	 //myModalForActiveUsers
	 modalRegFllPendingFees = document.getElementById("myModalRegFllPendingFees");
	 modalRegFllPendingFees.style.display = "block";
	 $("#memberIdRegFllPendingFees").val(memberId);
	 $("#memberNameRegFllPendingFees").val(memberName);
	 console.log(modalRegFllPendingFees);
	 $.getJSON("getPendingFeesDiscusionDetails",{memberId:memberId},function(res,status,xhr){
			
			 console.log(res);
			if(res.pendingFeesDiscusionDetails[0].memberId=="")
			{
				
				console.log("In opemmmm model",res);
				$("#memberIdRegFllPendingFees").val(memberId);
				$("#memberNameRegFllPendingFees").val(memberName);	
				$("#replyDateRegFllPendingFees").val("");
				$("#discussionRegFllPendingFees").val("");
				let per = $("#loggedUser").val();
				$("#calledByRegFllPendingFees").val(per);	
				return;
				
			}
			else
			{
				console.log("In opemmmm ",res);
				$("#memberIdRegFllPendingFees").val(res.pendingFeesDiscusionDetails[0].memberId);
				$("#memberNameRegFllPendingFees").val(res.pendingFeesDiscusionDetails[0].memberName);
				$("#replyDateRegFllPendingFees").val(res.pendingFeesDiscusionDetails[0].replyDate);
				$("#discussionRegFllPendingFees").val(res.pendingFeesDiscusionDetails[0].discussion);
				$("#calledByRegFllPendingFees").val(res.pendingFeesDiscusionDetails[0].calledBy);	
			}
			
		});
	
}

// When the user clicks on <span> (x), close the modal
 function closeModelRegFllPendingFees() {
	 document.getElementById("myModalRegFllPendingFees").style.display = "none";
	 $("#memberIdRegFllPendingFees").val("");
	 $("#memberNameRegFllPendingFees").val("");
	 $("#replyDateRegFllPendingFees").val("");
	 $("#discussionRegFllPendingFees").val("");
	 $("#calledByRegFllPendingFees").val("");
}


// When the user clicks anywhere outside of the modal, close it
window.onclick = function(event) {
  if (event.target == modal) {
    modal.style.display = "none";
    $("#memberIdRegFllPendingFees").val("");
	 $("#memberNameRegFllPendingFees").val("");
	 $("#replyDateRegFllPendingFees").val("");
	 $("#discussionRegFllPendingFees").val("");
	 $("#calledByRegFllPendingFees").val("");
  }
}
					function postDiscussioValuesRegFllPendingFees()
					{
						 data={
							 memberId :$("#memberIdRegFllPendingFees").val(),
							 memberName :$("#memberNameRegFllPendingFees").val(),
							 replyDate :$("#replyDateRegFllPendingFees").val(),
							 discussion :$("#discussionRegFllPendingFees").val(),
							 calledBy :$("#calledByRegFllPendingFees").val()
							 
						}
						console.log(data);
						$.post("pendingFeesDiscusionDetails",data,function(res){
					
							console.log(data);
						});
						location.reload();
						closeModel();
					}
</script>
 
<script>
// Get the modal for Active Users-----------------------------------------------------------------
	var modalAct;
 function openModelAct(memberId,memberName) {
 
	 modalAct = document.getElementById("myModalForActiveUsers");
	 modalAct.style.display = "block";
	 $("#memberIdAct").val(memberId);
	 $("#memberNameAct").val(memberName);
	  
	 $.getJSON("getActiveUsersDiscusionDetailsList",{memberId:memberId},function(res,status,xhr){
			
			 console.log(res.activeUsersDiscusionDetails[0]);
			if(res.activeUsersDiscusionDetails[0].memberId=="")
			{
				
				console.log("In opemmmm model",res);
				$("#memberIdAct").val(memberId);
				$("#memberNameAct").val(memberName);	
				$("#replyDateAct").val("");
				$("#discussionAct").val("");
				let per = $("#loggedUser").val();
				$("#calledByAct").val(per);	
				return;
				
			}
			else
			{
				console.log("In opemmmm ",res);
				$("#memberIdAct").val(res.activeUsersDiscusionDetails[0].memberId);
				$("#memberNameAct").val(res.activeUsersDiscusionDetails[0].memberName);
				$("#replyDateAct").val(res.activeUsersDiscusionDetails[0].replyDate);
				$("#discussionAct").val(res.activeUsersDiscusionDetails[0].discussion);
				$("#calledByAct").val(res.activeUsersDiscusionDetails[0].calledBy);	
			}
			
		}); 
	
}

// When the user clicks on <span> (x), close the modal
 function closeModelAct() {
	 document.getElementById("myModalForActiveUsers").style.display = "none";
	 $("#memberIdAct").val("");
	 $("#memberNameAct").val("");
	 $("#replyDateAct").val("");
	 $("#discussionAct").val("");
	 $("#calledByAct").val("");
}


// When the user clicks anywhere outside of the modal, close it
window.onclick = function(event) {
  if (event.target == modalAct) {
	 modalAct.style.display = "none";
      
  }
}
</script>
 <script>
					function postDiscussioValuesAct()
					{
						 data={
							 memberId :$("#memberIdAct").val(),
							 memberName :$("#memberNameAct").val(),
							 replyDate :$("#replyDateAct").val(),
							 discussion :$("#discussionAct").val(),
							 calledBy :$("#calledByAct").val()
							 
						}
						console.log(data);
						$.post("activeUsersDiscusionDetails",data,function(res){
					
							console.log(data);
							
						});
						location.reload();
						closeModelAct();
					}
 </script>

<script>
// Get the modal for RegMembers-----------------------------------------------------------------
	var modalRegMembers;
 function openModelRegMembers(memberId,memberName) {
 
	 modalRegMembers = document.getElementById("myModalForRegMembers");
	 modalRegMembers.style.display = "block";
	 $("#memberIdRegMembers").val(memberId);
	 $("#memberNameRegMembers").val(memberName);
	  
	 $.getJSON("getActiveUsersDiscusionDetailsList",{memberId:memberId},function(res,status,xhr){
			
			 console.log(res.activeUsersDiscusionDetails[0]);
			if(res.activeUsersDiscusionDetails[0].memberId=="")
			{
				
				console.log("In opemmmm model",res);
				$("#memberIdRegMembers").val(memberId);
				$("#memberNameRegMembers").val(memberName);	
				$("#replyDateRegMembers").val("");
				$("#discussionRegMembers").val("");
				let per = $("#loggedUser").val();
				$("#calledByRegMembers").val(per);	
				return;
				
			}
			else
			{
				console.log("In opemmmm ",res);
				$("#memberIdRegMembers").val(res.activeUsersDiscusionDetails[0].memberId);
				$("#memberNameRegMembers").val(res.activeUsersDiscusionDetails[0].memberName);
				$("#replyDateRegMembers").val(res.activeUsersDiscusionDetails[0].replyDate);
				$("#discussionRegMembers").val(res.activeUsersDiscusionDetails[0].discussion);
				$("#calledByRegMembers").val(res.activeUsersDiscusionDetails[0].calledBy);	
			}
			
		}); 
	
}

// When the user clicks on <span> (x), close the modal
 function closeModelRegMembers() {
	 document.getElementById("myModalForRegMembers").style.display = "none";
	 $("#memberIdRegMembers").val("");
	 $("#memberNameRegMembers").val("");
	 $("#replyDateRegMembers").val("");
	 $("#discussionRegMembers").val("");
	 $("#calledByRegMembers").val("");
}


// When the user clicks anywhere outside of the modal, close it
window.onclick = function(event) {
  if (event.target == modalRegMembers) {
	 modalRegMembers.style.display = "none";
      
  }
}
</script>
 <script>
					function postDiscussioValuesRegMembers()
					{
						 data={
							 memberId :$("#memberIdRegMembers").val(),
							 memberName :$("#memberNameRegMembers").val(),
							 replyDate :$("#replyDateRegMembers").val(),
							 discussion :$("#discussionRegMembers").val(),
							 calledBy :$("#calledByRegMembers").val()
							 
						}
						console.log(data);
						$.post("activeUsersDiscusionDetails",data,function(res){
					
							console.log(data);
							
						});
						closeModelRegMembers();
						location.reload();
					}
 </script>

