...
$("#document").ready(function(){ 
	...
	var id=	$("#dispMemberId").val();
	if(id!=0)
	{
		openFromSearchMember();
	}
	...
	//transactionDetail:{checkTransactionDetails:true} removed from jQuery.validator.addClassRules
	//Validation for form
		      ...
		      /*  var isConfirm= confirm(details+"\nTotal : Rs. "+amount+" /- \n If this is correct, click OK. \n To edit, click Cancel.");
		   	...
		   	function(isConfirm){
		   		form.action="makePayment.action";	
		   	 	sessionStorage.paymentActionType="makePayment";
		   		if (isConfirm){		   			
		   			form.submit();
		   			//document.getElementById("memberFeesForm").action="makePayment";
		   			//document.getElementById("memberFeesForm").submit();
		   		} else {
		   			//skip 
		   		}
		   	});   
	...
	//cooment that code later 
	 $("#totalPendingFees").css("color","red");
	 /* $("#receiptNo").blur(function(){
		  let receiptId=$("#receiptNo").val();
		  $.getJSON('checkReciptIdExist',{receiptId:receiptId}).done(function(data){
			  if(data.receiptNoList.length>0){
				  alert("Receipt No exist already! Please try another one.");
			  }
			  //console.log(data);
		  });
	 }); */
...
var openFromSearchMember=function()
{
	onMemberIdChange();						//Fetch Members Details On Change of Member Id
};
...
var isNewReceipt=false;
			...
			$.getJSON('fetchMemberPaymentDetails',{memberId : memberId})			//Json To Fetch Data
				...
				{				
					...
					if(data.memberDetails[0] != undefined){					
						...
						$("#memberNamee").val(data.memberDetails[0].firstName+" "+data.memberDetails[0].middleName+" "+data.memberDetails[0].lastName);
						$("#contactNo1").val(data.memberDetails[0].contactNo);
						$("#contactNo3").val(data.memberDetails[0].contactNo);
	...
	var assignFeeData = function(jsondata){
		 ...
		 */
				...
				$('<td><input type="text" tabindex="-1" style="width:80px;" class="ledgerAlreadyPaidFees" id="ledgerAlreadyPaidFees'+id+'" name="ledgerFeesPaids['+id+'].ledgerAlreadyPaidFees" value="'+ledgerAlreadyPaidFees+'" readonly="true" /></td>').appendTo(row);
				...
				$('<td><input type="text" tabindex="-1" style="width:80px;" class="ledgerPendingFees" id="ledgerPendingFees'+id+'" name="ledgerFeesPaids['+id+'].ledgerPendingFees" value="'+data[i].ledgerPendingFees+'" readonly="readonly" /></td>').appendTo(row);
...
var calculate = function(){
   ...
   //$("#totalPendingFees").val(totalPendingFees.toFixed(2));
    ...
    $.each($('.ledgerPaidFees'), function() {
		...
		if(!isNaN($("#ledgerAlreadyPaidFees"+id).val()) && ($("#ledgerAlreadyPaidFees"+id).val()).length!=0 || ($("#discount").val())>=0) 
			ledgerPendingFees=ledgerPendingFees - $("#ledgerAlreadyPaidFees"+id).val()-$("#discount").val();
		
		if(!isNaN(this.value) && this.value.length!=0) 
			ledgerPendingFees -= parseFloat(this.value);
		
	//	if(($("#discount").val())>0 && (totalPendingFees>($("#discount").val())))
		//{
			//ledgerPendingFees=  $("#totalPaidFees").val()-$("#discount").val();
		//}
...
<body onclick="parent_disable();">
	...
	<div>
		...
		<s:textfield name="receiptStatus" id="receiptStatus" value="new" style="display:none;"/>
		<s:textfield name="regFeesStatus" id="regFeesStatus" value="" style="display:none;"/>
				...
				<table>
					<tr>
						...
						<td><label>Member Name</label></td>
							<s:hidden name="memberName" id="memberNamee" value="%{name}"/>
						...
						<td valign="top"><s:textfield tabindex="-1" name="contactNo" id="contactNo1"
								value="%{contactNumber}"/>
								<s:hidden name="contactN" id="contactNo3" value="%{contactNumber}" />  
			...
			<fieldset id="receiptData" style="display: none;">
				<table>
					...
					<tr>
						<td><s:textfield tabindex="-1" name="receiptNo" value="%{displayReceiptNo}" id="receiptNo" readonly="true"/>
	...
	<!-- change   --> 					
			...
			<fieldset id="paymentData" style="display: none;">
				<table>
					<tr>
						...
						<td><label>Cheque/Trans/Phone No.</label></td>
			...
			<fieldset id="saveData">		   		
				<s:set var="paymentActionType" id="paymentActionType" value="%{'makePayment'}" scope="session"/>
