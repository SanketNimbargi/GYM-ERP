...
var auditDataList = [];
	$("#document").ready(function(){	
	  ...
	  $("#getList").click(function(){
		  ...
		  $.getJSON('fetchMemberList',{fromDate : fromDate, toDate : toDate, feeStatus : feeStatus}).done(function(data){				 
				$('<div class="errors" id="message" style="width: 400px; display: none;"></div>)').appendTo($("#errorContainer"));
				if(data.auditMemberList == undefined || parseInt(data.auditMemberList.length)==0 ){
					...
					$('<table class="display" id="auditDetailTable">'+
						...
						'<th>Mobile Number</th><th>Ledger Total Amount</th><th>Discount</th><th>Gross Amount</th><th>Registration Fees</th><th>Tax %</th><th>Tax Type</th><th>Paid Fees</th>'+
					...
					createTable($("#auditDetailTable tbody"),data.auditMemberList);						
				}
		 })
...
/* 	<th>Sr.No.</th>	
	...
	function pushTableData(){
		...
		auditDataList.length=0;
		  $("#checkboxlist input:checked").each(function() {	
				...
				var name=$('#tr'+rowId).find('td').eq(2).text().replace(/\s/g, '');
				...
				var grossAmount=$('#tr'+rowId).find('td').eq(7).text().replace(/\s/g, '');	
				var registrationFee=$('#tr'+rowId).find('td').eq(8).text().replace(/\s/g, '');
				
				
				var tax=$('#tr'+rowId).find('td').eq(9).text().replace(/\s/g, '');
				var taxType=$('#tr'+rowId).find('td').eq(10).text().replace(/\s/g, '');	
				var finalPaidFees=$('#tr'+rowId).find('td').eq(11).text().replace(/\s/g, '');	
				
				var firstName=$('#tr'+rowId).find('td').eq(12).text().replace(/\s/g, '');
				var middleName=$('#tr'+rowId).find('td').eq(13).text().replace(/\s/g, '');
				var lastName=$('#tr'+rowId).find('td').eq(14).text().replace(/\s/g, '');
				
				auditDataList.push({receiptDate:receiptDate,name:name,finalReceiptNo:finalReceiptNo,
					contactNo:contactNo,
					ledgerTotalFees:ledgerTotalFees,
					discount:discount,
					grossAmount:grossAmount,registrationFee:registrationFee,		
					tax:tax,taxType:taxType,finalPaidFees:finalPaidFees,
	...
	var createTable = function(tableName,data){
		...
		for (var i = 0; i < data.length; i++) {
			...
			$('<td></td>').text(convertToUserDate(data[i].receiptDate)).appendTo(row);
			$('<td style="width: 50%"></td>').text(data[i].name).appendTo(row);
			$('<td></td>').text(data[i].finalReceiptNo).appendTo(row);	
			
			
			
			$('<td></td>').text(data[i].contactNo).appendTo(row);
			$('<td></td>').text(data[i].ledgerTotalFees).appendTo(row); /* totalMount */
			$('<td></td>').text(data[i].discount).appendTo(row);
			$('<td></td>').text(data[i].grossAmount).appendTo(row);	
			$('<td></td>').text(data[i].registrationFee).appendTo(row);
			
			$('<td></td>').text(data[i].tax).appendTo(row);
			$('<td></td>').text(data[i].taxType).appendTo(row);
			$('<td></td>').text(data[i].finalPaidFees).appendTo(row);
		...
		<s:form id="auditReportForm" theme="css_xhtml">
		 <s:hidden id="auditDataList" name="auditDataList" value=""/>
			...
			<table>
					 <tr>						
						<td>
							<s:submit action="auditReportPrintAction" id="auditReportAction" align="center" value="Audit Report"  />
					   </td>						
					</tr>
				</table>	
			</fieldset>
